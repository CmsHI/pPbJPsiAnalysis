...
int cndr_rootAna_meanPt_wBkg_0(char *strBin = "PAS"){
...
*/
	...
	Double_t yArr[nRapTmp] = {1.93, 1.5};
...
*/
			...
			cmulti[in][ir]->Update();
			cmulti[in][ir]->SaveAs(Form("dir_meanPt_n%d_%s/meanPt_%s_%d_%d_wBkg_0.pdf",nbin,strBin,strBin,in,ir));
			//cmulti[in][ir]->Clear();
		}
	}
	
	/// Save as a root file
	TFile *outFile = new TFile(Form("dir_meanPt_n%d_%s/meanPt_%s_wBkg_0.root",nbin,strBin,strBin),"RECREATE");
