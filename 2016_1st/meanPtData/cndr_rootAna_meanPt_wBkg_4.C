...
int cndr_rootAna_meanPt_wBkg_4(char *strBin = "PAS"){
...
*/
			...
			cmulti[in][ir]->Update();
			cmulti[in][ir]->SaveAs(Form("dir_meanPt_n%d_%s/meanPt_%s_%d_%d_wBkg_4.pdf",nbin,strBin,strBin,in,ir));
			//cmulti[in][ir]->Clear();
		}
	}
	
	/// Save as a root file
	TFile *outFile = new TFile(Form("dir_meanPt_n%d_%s/meanPt_%s_wBkg_4.root",nbin,strBin,strBin),"RECREATE");
