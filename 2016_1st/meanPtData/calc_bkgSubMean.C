...
int calc_bkgSubMean(){
	...
	TFile *fIn = new TFile("./dir_meanPt_n20_8rap9pt/meanPt_8rap9pt_wBkg.root"); //newres && nbin=20
	...
	for (int in=0; in<nEt; in++){
		for (int ir=0; ir<nRap; ir++ ) {
			for (int ipt=0; ipt<nPt; ipt++) {
				hMeanPt[in][ir][ipt] = (TH1D*)fIn->Get(Form("hMeanPt_%d_%d_%d",in,ir,ipt));
				hMeanPtBkg[in][ir][ipt] = (TH1D*)fIn->Get(Form("hMeanPtBkg_%d_%d_%d",in,ir,ipt));
				...
				hMeanPtSub[in][ir][ipt] = (TH1D*)hMeanPt[in][ir][ipt]->Clone(Form("hMeanPtSub_%s",histName[i].Data()));
