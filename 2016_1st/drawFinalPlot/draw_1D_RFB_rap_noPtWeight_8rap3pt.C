...
void draw_1D_RFB_rap_noPtWeight_8rap3pt(char* dirName = "8rap3pt", int runCode=2, bool isPrompt = false)
{
...
//	latex->DrawLatex(0.56, 0.20, lumistring.c_str());
	...
	c1->Update();
//	c1->SaveAs(Form("RFB_%s_noPtWeight/RFB_rap_isPrompt%d_%s.pdf",dirName,(int)isPrompt,runstring.c_str()));
//	c1->SaveAs(Form("RFB_%s_noPtWeight/RFB_rap_isPrompt%d_%s.png",dirName,(int)isPrompt,runstring.c_str()));
	
	///////////////////////////////////////////////////////////////////
/*
	// save as a root file
	TFile *outFile = new TFile(Form("RFB_%s_noPtWeight/RFB_rap_isPrompt%d.root",dirName,(int)isPrompt),"RECREATE");
	outFile->cd();
