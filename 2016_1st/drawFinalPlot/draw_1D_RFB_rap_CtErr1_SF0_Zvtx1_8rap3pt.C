...
void draw_1D_RFB_rap_CtErr1_SF0_Zvtx1_8rap3pt(char* dirName = "8rap3pt", int runCode=2, bool isPrompt = true)
{
...
//	cout << "nRap = " << nRap << endl;
	...
	//8rap3pt
	const int lowpt_init=1;
	const int highpt_init=2;	
	//8rap9pt
...
//	latex->DrawLatex(0.56, 0.20, lumistring.c_str());
	...
	if (isPrompt) globtex->DrawLatex(0.28, 0.86, "Prompt J/#psi");
	else globtex->DrawLatex(0.28, 0.86, "Non-prompt J/#psi");
	CMS_lumi( c1, iPeriod, iPos );
	c1->Update();
	c1->SaveAs(Form("RFB_%s_CtErr1_SF0_Zvtx1/RFB_rap_isPrompt%d_%s.pdf",dirName,(int)isPrompt,runstring.c_str()));
	c1->SaveAs(Form("RFB_%s_CtErr1_SF0_Zvtx1/RFB_rap_isPrompt%d_%s.png",dirName,(int)isPrompt,runstring.c_str()));
	
	///////////////////////////////////////////////////////////////////
	// save as a root file
	TFile *outFile = new TFile(Form("RFB_%s_CtErr1_SF0_Zvtx1/RFB_rap_isPrompt%d.root",dirName,(int)isPrompt),"RECREATE");
	outFile->cd();
