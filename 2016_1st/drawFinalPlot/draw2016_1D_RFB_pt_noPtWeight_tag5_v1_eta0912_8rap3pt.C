...
void draw2016_1D_RFB_pt_noPtWeight_tag5_v1_eta0912_8rap3pt(char* dirName = "8rap3pt", int runCode=0, bool isPrompt=true)
{
...
*/	
  ...
  //gStyle->SetPadRightMargin(0.075);
	...
	TFile * f2D = new TFile(Form("../fittingResult/total2Dhist_%s_noPtWeight_tag5_v1_eta0912.root",dirName));
...
//		legBR -> AddEntry(gRFB[nRapRFB-1-iy],Form("%s",rapAbsArr[nRapRFB-1-iy].c_str()),"lp");
	...
	c1->Update();
	c1->SaveAs(Form("RFB_%s_noPtWeight_tag5_v1_eta0912/RFB_pt_isPrompt%d_%s.pdf",dirName,(int)isPrompt,runstring.c_str()));
	c1->SaveAs(Form("RFB_%s_noPtWeight_tag5_v1_eta0912/RFB_pt_isPrompt%d_%s.png",dirName,(int)isPrompt,runstring.c_str()));
	
	///////////////////////////////////////////////////////////////////
	// save as a root file
	TFile *outFile = new TFile(Form("RFB_%s_noPtWeight_tag5_v1_eta0912/RFB_pt_isPrompt%d.root",dirName,(int)isPrompt),"RECREATE");
	outFile->cd();
	for (int iy=0; iy<nRapRFB; iy++){
		gRFB_sys[iy]->Write();	
		gRFB[iy]->Write();	
