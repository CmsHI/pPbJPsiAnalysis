...
void draw_corrYield_compRatio_two(bool isPrompt = true, bool is1st = true, char* dirName02 = "draw_compRatio_two_cutG", bool isNoError=true, bool isLog=false)
{
	...
	//TFile * f02;
	TFile * f03;
	char* f01name="original cut";
	//char* f02name="cut C (tighter)";
	char* f03name="new cut";
	...
	f01 = new TFile("./corrYield_oldcut_noWeight/corrYield_pt_isLog0_noWeight.root");
	//f02 = new TFile(Form("./draw_1D_corrYield_newcut/8rap9pt2gev_%s_AccPt_newcut.root",strPrompt));
	f03 = new TFile("./corrYield_cutG_noWeight/corrYield_pt_isLog0_noWeight.root");
...
//	c3->Divide(4,2);
	...
	for (Int_t iy = 0; iy < nRap; iy++) {
		...
		SetHistStyle(h1D_AccPt01[iy],0,0);
		//SetHistStyle(h1D_AccPt01[iy],0,1);
		//SetHistStyle(h1D_AccPt02[iy],3,10);
		SetHistStyle(h1D_AccPt03[iy],5,13);
		h1D_AccPt01[iy]->GetXaxis()->SetTitle("p_{T} (GeV)");
		h1D_AccPt01[iy]->GetYaxis()->SetTitle("corrected Yield");
		if (!isLog) h1D_AccPt01[iy]->SetMinimum(0.);
//		h1D_AccPt01[iy]->SetMaximum(1.);
		h1D_AccPt01[iy]->GetXaxis()->SetRangeUser(0., 30.);
		h1D_AccPt01[iy]->Draw("pe");
//		h1D_AccPt02[iy]->Draw("pe same");
		h1D_AccPt03[iy]->Draw("pe same");
		if (is1st) {
		...
		latex->DrawLatex(0.53,0.65,sampleName);
		latex->DrawLatex(0.53,0.59,Form("%.2f < y_{lab} < %.2f",rapEdge[iy],rapEdge[iy+1]));
		if (iy==0) {
			legUR->AddEntry(h1D_AccPt01[iy],f01name,"p");
//			legUR->AddEntry(h1D_AccPt02[iy],f02name,"lp");
			legUR->AddEntry(h1D_AccPt03[iy],f03name,"p");
			legUR->Draw();
		}
		...
		hRatio[iy]->GetYaxis()->SetTitle(Form("[ %s ]/[ %s ]",f01name,f03name));
...
*/
	...
	c1->SaveAs(Form("%s/%s_corrYield_compDist_two_isLog%d.pdf",dirName02,strPrompt,(int)isLog));
	c2->SaveAs(Form("%s/%s_corrYield_compRatio_two_isLog%d.pdf",dirName02,strPrompt,(int)isLog));
