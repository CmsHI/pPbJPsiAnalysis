...
void draw_acc_compRatio_two(bool isPrompt = true, bool is1st = true, char* dirName02 = "draw_compRatio_two_cutG", bool isNoError=true)
{
	...
	f01 = new TFile("./acc_oldcut_noWeight/acc_pt_isLog0_noWeight.root");
	//f02 = new TFile(Form("./draw_1D_acc_newcut/8rap9pt2gev_%s_AccPt_newcut.root",strPrompt));
	//f03 = new TFile("./acc_cutB_noWeight/acc_pt_isLog0_noWeight.root");
	f03 = new TFile("./acc_cutG_noWeight/acc_pt_isLog0_noWeight.root");
...
//	TH1D *hRel[nRap]; // (f01-f02)/f02
	for (int iy=0; iy < nRap; iy ++){
		h1D_AccPt01[iy] = (TH1D*)f01->Get(Form("h1D_acc_PRMC_Pbp_%d",iy));
//		h1D_AccPt02[iy] = (TH1D*)f02->Get(Form("h1D_AccPt_%d",iy));
		h1D_AccPt03[iy] = (TH1D*)f03->Get(Form("h1D_acc_PRMC_Pbp_%d",iy));
...
*/
	...
	c1->SaveAs(Form("%s/%s_acc_compDist_two.pdf",dirName02,strPrompt));
