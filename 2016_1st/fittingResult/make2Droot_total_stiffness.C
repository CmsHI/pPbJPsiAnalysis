...
/////// main func. ///////
int make2Droot_total_stiffness(char *dirName = "8rap3pt", bool isStiff = false){
  ...
  using namespace std;
	...
	// *** with pt weight
	TFile *fAccPR = new TFile(Form("../AccEffWeight/AccAna_%s_PRMC_boosted_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	TFile *fAccNP = new TFile(Form("../AccEffWeight/AccAna_%s_NPMC_boosted_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	TH2D* h2D_Acc_pt_y_PR_Pbp = (TH2D*)fAccPR->Get("h2D_Acc_pt_y_Pbp"); //weighted!
	TH2D* h2D_Acc_pt_y_PR_pPb = (TH2D*)fAccPR->Get("h2D_Acc_pt_y_pPb");
	TH2D* h2D_Acc_pt_y_NP_Pbp = (TH2D*)fAccNP->Get("h2D_Acc_noPtWeight_pt_y_Pbp"); //un-weighted!
	TH2D* h2D_Acc_pt_y_NP_pPb = (TH2D*)fAccNP->Get("h2D_Acc_noPtWeight_pt_y_pPb");
	...
	TH2D* h2D_Acc_Den_pt_y_NP_Pbp = (TH2D*)fAccNP->Get("h2D_Den_pt_y_Pbp"); //WRONG (weighted)
	TH2D* h2D_Acc_Den_pt_y_NP_pPb = (TH2D*)fAccNP->Get("h2D_Den_pt_y_pPb"); //WRONG (weighted)
	...
	TH2D* h2D_Acc_Num_pt_y_NP_Pbp = (TH2D*)fAccNP->Get("h2D_Num_pt_y_Pbp"); //WRONG (weighted) 
	TH2D* h2D_Acc_Num_pt_y_NP_pPb = (TH2D*)fAccNP->Get("h2D_Num_pt_y_pPb"); //WRONG (weighted) 
	...
	/// *** with pt weight
	TFile *fEffPRPbp = new TFile(Form("../AccEffWeight/EffAna_%s_PRMCpythia_Pbp_CtErr1_SF1_Zvtx1_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	TFile *fEffPRpPb = new TFile(Form("../AccEffWeight/EffAna_%s_PRMCpythia_pPb_CtErr1_SF1_Zvtx1_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	TFile *fEffNPPbp = new TFile(Form("../AccEffWeight/EffAna_%s_NPMCpythia_Pbp_CtErr1_SF1_Zvtx1_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	TFile *fEffNPpPb = new TFile(Form("../AccEffWeight/EffAna_%s_NPMCpythia_pPb_CtErr1_SF1_Zvtx1_ptWeight_isStiff%d.root",dirName,(int)isStiff));
	//cout << "Eff file : " << fEffPRPbp << " " << fEffPRpPb << " "<<fEffNPPbp << " " << fEffNPpPb << endl; 
	TH2D* h2D_Eff_pt_y_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Eff_pt_y"); //weighted!
	TH2D* h2D_Eff_pt_y_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Eff_pt_y");
	TH2D* h2D_Eff_pt_y_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Eff_noPtWeight_pt_y"); //unweighted!
	TH2D* h2D_Eff_pt_y_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Eff_noPtWeight_pt_y");
	...
	TH2D* h2D_Eff_Den_pt_y_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Den_noPtWeight_pt_y");
	TH2D* h2D_Eff_Den_pt_y_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Den_noPtWeight_pt_y");
	...
	TH2D* h2D_Eff_Num_pt_y_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Num_noPtWeight_pt_y");
	TH2D* h2D_Eff_Num_pt_y_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Num_noPtWeight_pt_y");
	...
	////////////////////////////////////////////////
	////// save as a root file
	TFile *outFile = new TFile(Form("total2Dhist_%s_isStiff%d.root",dirName,(int)isStiff),"RECREATE");
	std::cout << "dirName: " << dirName << std::endl;
	cout << "total2Dhist_"<<dirName<<"_isStiff"<<(int)isStiff<<".root has been created :) " <<endl;	
