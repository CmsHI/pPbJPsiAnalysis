...
void draw_1D_rawYield_pt(char* dirName = "PAS", bool isLog=false)
{
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		//h1D_fit_NP_Pbp[iy] = h2D_fit_pt_y_NP_Pbp->ProjectionY(Form("h1D_fit_NP_Pbp_%d",iy),nbinsX-iy,nbinsX-iy);
...
//	}
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		//h1D_fit_PR_Pbp[iy]->GetXaxis()->SetRangeUser(2.,30.);
		h1D_fit_PR_Pbp[iy]->GetXaxis()->SetRangeUser(0.,30.);
		h1D_fit_PR_Pbp[iy]->GetYaxis()->SetTitle("raw yield");
//		h1D_fit_PR_Pbp[iy]->GetYaxis()->SetRangeUser(0,1.0);
...
//			legBR -> AddEntry(h1D_fit_PR_Pbp[iy],"1st run (Pbp)","lp");
//			legBR -> AddEntry(h1D_fit_NP_Pbp[iy],"2nd run (pPb)","lp");
			legBR -> AddEntry(h1D_fit_PR_Pbp[iy],"Prompt J/#psi","lp");
			legBR -> AddEntry(h1D_fit_NP_Pbp[iy],"Non-prompt J/#psi","lp");
			//legBR->Draw();
		}
...
//	legBR->Draw();
	...
	c_multi->SaveAs(Form("dir_1D_%s/rawYield_pt_isLog%d.pdf",dirName,(int)isLog));
	//c_multi->SaveAs(Form("dir_1D_%s_noPtWeight/rawYield_pt_isLog%d.pdf",dirName,(int)isLog));
