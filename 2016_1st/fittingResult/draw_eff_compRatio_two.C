...
void draw_eff_compRatio_two(bool isPrompt = true, bool is1st = true, char* dirName02 = "draw_compRatio_two_cutG", bool isNoError=true)
{
	...
	f01 = new TFile("./eff_oldcut_noWeight/eff_pt_isLog0_noWeight.root");
	//f02 = new TFile(Form("./draw_1D_eff_newcut/8rap9pt2gev_%s_AccPt_newcut.root",strPrompt));
	//f03 = new TFile("./eff_cutB_noWeight/eff_pt_isLog0_noWeight.root");
	f03 = new TFile("./eff_cutG_noWeight/eff_pt_isLog0_noWeight.root");
...
//	TH1D *hRel[nRap]; // (f01-f02)/f02
	for (int iy=0; iy < nRap; iy ++){
		h1D_AccPt01[iy] = (TH1D*)f01->Get(Form("h1D_eff_PRMC_Pbp_%d",iy));
//		h1D_AccPt02[iy] = (TH1D*)f02->Get(Form("h1D_AccPt_%d",iy));
		h1D_AccPt03[iy] = (TH1D*)f03->Get(Form("h1D_eff_PRMC_Pbp_%d",iy));
...
//	c3->Divide(4,2);
	...
	for (Int_t iy = 0; iy < nRap; iy++) {
		...
		h1D_AccPt01[iy]->GetYaxis()->SetTitle("Efficiency");
