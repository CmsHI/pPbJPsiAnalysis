...
/////// main func. ///////
int make2Droot_total_noPtWeight_check5_oldacc_oldeta_private_lowerptlimit_bfracControl(char *dirName = "8rap3pt", char *dateName = "no"){
  ...
  using namespace std;
	...
	// *** without pt weight
	TFile *fAccPR = new TFile(Form("../Acceptance/AccAna_%s_PRMC_boosted.root",dirName));
	TFile *fAccNP = new TFile(Form("../Acceptance/AccAna_%s_NPMC_boosted.root",dirName));
	...
	// *** without pt weight
	TFile *fEffPRPbp = new TFile(Form("../Efficiency/EffAna_%s_PRMCpythia_Pbp_CtErr1_SF1_Zvtx1_check5_oldacc_oldeta_private_lowerptlimit.root",dirName));
	TFile *fEffPRpPb = new TFile(Form("../Efficiency/EffAna_%s_PRMCpythia_pPb_CtErr1_SF1_Zvtx1_check5_oldacc_oldeta_private_lowerptlimit.root",dirName));
	TFile *fEffNPPbp = new TFile(Form("../Efficiency/EffAna_%s_NPMCpythia_Pbp_CtErr1_SF1_Zvtx1_check5_oldacc_oldeta_private_lowerptlimit.root",dirName));
	TFile *fEffNPpPb = new TFile(Form("../Efficiency/EffAna_%s_NPMCpythia_pPb_CtErr1_SF1_Zvtx1_check5_oldacc_oldeta_private_lowerptlimit.root",dirName));
	...
	TFile* fFitPbp = new TFile(Form("./2Dhist_fitRes_%s_bfracControl_Pbp.root",dirName));
	TFile* fFitpPb = new TFile(Form("./2Dhist_fitRes_%s_bfracControl_pPb.root",dirName));
	...
	////////////////////////////////////////////////
	////// save as a root file
	TFile *outFile = new TFile(Form("total2Dhist_%s_noPtWeight_check5_oldacc_oldeta_private_lowerptlimit_bfracControl.root",dirName),"RECREATE");
	std::cout << "dirName: " << dirName << std::endl;
	cout << "total2Dhist_"<<dirName<<"_noPtWeight_check5_oldacc_oldeta_private_lowerptlimit_bfracControl.root has been created :) " <<endl;	
