...
/////// main func. ///////
int make2Droot_total_noPtWeight_oldacccut_allTag5(char *dirName = "8rap3pt", char *dateName = "no"){
  ...
  using namespace std;
	...
	// *** without pt weight
	TFile *fAccPR = new TFile(Form("../Acceptance/AccAna_%s_PRMC_boosted_oldacccut.root",dirName));
	TFile *fAccNP = new TFile(Form("../Acceptance/AccAna_%s_NPMC_boosted_oldacccut.root",dirName));
	...
	// *** without pt weight
	TFile *fEffPRPbp = new TFile(Form("../Efficiency/EffAna_%s_PRMCpythia_Pbp_CtErr1_SF1_Zvtx1_oldacccut_allTag5.root",dirName));
	TFile *fEffPRpPb = new TFile(Form("../Efficiency/EffAna_%s_PRMCpythia_pPb_CtErr1_SF1_Zvtx1_oldacccut_allTag5.root",dirName));
	TFile *fEffNPPbp = new TFile(Form("../Efficiency/EffAna_%s_NPMCpythia_Pbp_CtErr1_SF1_Zvtx1_oldacccut_allTag5.root",dirName));
	TFile *fEffNPpPb = new TFile(Form("../Efficiency/EffAna_%s_NPMCpythia_pPb_CtErr1_SF1_Zvtx1_oldacccut_allTag5.root",dirName));
	...
	TFile* fFitPbp = new TFile(Form("./2Dhist_fitRes_%s_oldacccut_Pbp.root",dirName));
	TFile* fFitpPb = new TFile(Form("./2Dhist_fitRes_%s_oldacccut_pPb.root",dirName));
	...
	////////////////////////////////////////////////
	////// save as a root file
	TFile *outFile = new TFile(Form("total2Dhist_%s_noPtWeight_oldacccut_allTag5.root",dirName),"RECREATE");
	std::cout << "dirName: " << dirName << std::endl;
	cout << "total2Dhist_"<<dirName<<"_noPtWeight_oldacccut_allTag5.root has been created :) " <<endl;	
