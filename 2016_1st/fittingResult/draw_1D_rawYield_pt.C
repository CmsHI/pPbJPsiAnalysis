...
void draw_1D_rawYield_pt(char* dirName = "8rap9pt", bool is1st= true, bool isLog=false)
{
	...
	TH2D* h2D_fit_pt_y_NP_Pbp;
	TH2D* h2D_fit_pt_y_PR_pPb;
	TH2D* h2D_fit_pt_y_NP_pPb;
	h2D_fit_pt_y_PR_Pbp = (TH2D*)f2D->Get("h2D_fit_pt_y_PR_Pbp");
	h2D_fit_pt_y_NP_Pbp = (TH2D*)f2D->Get("h2D_fit_pt_y_NP_Pbp");
	h2D_fit_pt_y_PR_pPb = (TH2D*)f2D->Get("h2D_fit_pt_y_PR_pPb");
	h2D_fit_pt_y_NP_pPb = (TH2D*)f2D->Get("h2D_fit_pt_y_NP_pPb");
	cout << "h2D_fit_pt_y_PR_Pbp = " << h2D_fit_pt_y_PR_Pbp << endl;
	cout << "h2D_fit_pt_y_NP_Pbp = " << h2D_fit_pt_y_NP_Pbp << endl;
	cout << "h2D_fit_pt_y_PR_pPb = " << h2D_fit_pt_y_PR_pPb << endl;
	cout << "h2D_fit_pt_y_NP_pPb = " << h2D_fit_pt_y_NP_pPb << endl;
	...
	TH1D* h1D_fit_NP_Pbp[nbinsX]; 
	TH1D* h1D_fit_PR_pPb[nbinsX]; 
	TH1D* h1D_fit_NP_pPb[nbinsX]; 
	
	// iy=0 refers to forwards !!! (ordering here)
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		h1D_fit_PR_Pbp[iy] = h2D_fit_pt_y_PR_Pbp->ProjectionY(Form("h1D_fit_PR_Pbp_%d",iy),iy+1,iy+1);
		h1D_fit_NP_Pbp[iy] = h2D_fit_pt_y_NP_Pbp->ProjectionY(Form("h1D_fit_NP_Pbp_%d",iy),iy+1,iy+1); //tmp
		h1D_fit_PR_pPb[iy] = h2D_fit_pt_y_PR_pPb->ProjectionY(Form("h1D_fit_PR_pPb_%d",iy),nbinsX-iy,nbinsX-iy);
		h1D_fit_NP_pPb[iy] = h2D_fit_pt_y_NP_pPb->ProjectionY(Form("h1D_fit_NP_pPb_%d",iy),nbinsX-iy,nbinsX-iy);
	}
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		if (iy>=1 && iy<=6) {
			...
			h1D_fit_NP_Pbp[iy]->SetBinContent(1,0);
			h1D_fit_NP_Pbp[iy]->SetBinError(1,0);
			h1D_fit_PR_pPb[iy]->SetBinContent(1,0);
			h1D_fit_PR_pPb[iy]->SetBinError(1,0);
			h1D_fit_NP_pPb[iy]->SetBinContent(1,0);
			h1D_fit_NP_pPb[iy]->SetBinError(1,0);
		}
		if (iy>=2 && iy<=5) {
			...
			h1D_fit_NP_Pbp[iy]->SetBinContent(2,0);
			h1D_fit_NP_Pbp[iy]->SetBinContent(3,0);
			h1D_fit_NP_Pbp[iy]->SetBinError(2,0);
			h1D_fit_NP_Pbp[iy]->SetBinError(3,0);
			...
			h1D_fit_NP_pPb[iy]->SetBinContent(2,0);
			h1D_fit_NP_pPb[iy]->SetBinContent(3,0);
			h1D_fit_NP_pPb[iy]->SetBinError(2,0);
			h1D_fit_NP_pPb[iy]->SetBinError(3,0);
		}
		if (iy>=2 && iy<=4) {
			h1D_fit_PR_Pbp[iy]->SetBinContent(4,0);
			h1D_fit_PR_Pbp[iy]->SetBinError(4,0);
			h1D_fit_NP_Pbp[iy]->SetBinContent(4,0);
			h1D_fit_NP_Pbp[iy]->SetBinError(4,0);
			h1D_fit_PR_pPb[iy]->SetBinContent(4,0);
			h1D_fit_PR_pPb[iy]->SetBinError(4,0);
			h1D_fit_NP_pPb[iy]->SetBinContent(4,0);
			h1D_fit_NP_pPb[iy]->SetBinError(4,0);
		}
	}
...
//	}
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		SetHistStyle(h1D_fit_NP_Pbp[iy],2,10);
		...
		h1D_fit_NP_Pbp[iy]->Draw("pe same");
		if (iy==0) {
			legBRPbp -> SetHeader("Pbp");
			legBRPbp -> AddEntry(h1D_fit_PR_Pbp[iy],"Prompt J/#psi","lp");
			legBRPbp -> AddEntry(h1D_fit_NP_Pbp[iy],"Non-prompt J/#psi","lp");
			legBRPbp->Draw();
		}
		...
		SetHistStyle(h1D_fit_NP_pPb[iy],2,10);
		...
		h1D_fit_NP_pPb[iy]->Draw("pe same");
		if (iy==0) {
			legBRpPb -> SetHeader("pPb");
			legBRpPb -> AddEntry(h1D_fit_PR_Pbp[iy],"Prompt J/#psi","lp");
			legBRpPb -> AddEntry(h1D_fit_NP_Pbp[iy],"Non-prompt J/#psi","lp");
			legBRpPb ->Draw();
		}
		latex->DrawLatex(0.49,0.80,Form("%s",rapArr[iy].c_str()));
	}

	//c_Pbp->SaveAs(Form("dir_1D_%s/rawYield_pt_isLog%d_Pbp.pdf",dirName,(int)isLog));
	//c_pPb->SaveAs(Form("dir_1D_%s/rawYield_pt_isLog%d_pPb.pdf",dirName,(int)isLog));
	c_Pbp->SaveAs(Form("dir_1D_%s_noPtWeight/rawYield_pt_isLog%d_Pbp.pdf",dirName,(int)isLog));
	c_pPb->SaveAs(Form("dir_1D_%s_noPtWeight/rawYield_pt_isLog%d_pPb.pdf",dirName,(int)isLog));

	// root file
	//TFile *fOut = new TFile(Form("dir_1D_%s/rawYield_pt_isLog%d.root",dirName,(int)isLog),"RECREATE");
	TFile *fOut = new TFile(Form("dir_1D_%s_noPtWeight/rawYield_pt_isLog%d.root",dirName,(int)isLog),"RECREATE");
	fOut->cd();
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		h1D_fit_PR_Pbp[iy]->Write();
		h1D_fit_NP_Pbp[iy]->Write();
		h1D_fit_PR_pPb[iy]->Write();
		h1D_fit_NP_pPb[iy]->Write();
	}
	fOut->Close();
	return;
