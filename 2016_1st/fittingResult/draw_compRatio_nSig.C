...
void draw_compRatio_nSig(bool is1st = true, char* dirName02 = "dir_compRatio_PAS_NEW", bool isNoError=false)
{
	...
	f01 = new TFile("./dir_1D_PAS/nSig_pt_isLog0.root");
	f02 = new TFile("./dir_1D_8rap9pt_noPtWeight/nSig_pt_isLog0.root");
...
//	TH1D *hRel[nRap]; // (f01-f02)/f02
	for (int iy=0; iy < nRap; iy ++){
		h1D_AccPt01[iy] = (TH1D*)f01->Get(Form("h1D_fit_nSig_%s_%d",str1st, iy));
		h1D_AccPt02[iy] = (TH1D*)f02->Get(Form("h1D_fit_nSig_%s_%d",str1st, iy));
...
*/
	...
	c1->SaveAs(Form("%s/nSig_compDist_PAS_NEW_%s_%s.pdf",dirName02,strPrompt,str1st));
	c2->SaveAs(Form("%s/nSig_compRatio_PAS_NEW_%s_%s.pdf",dirName02,strPrompt,str1st));
