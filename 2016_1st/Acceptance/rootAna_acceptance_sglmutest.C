...
// MrapNpt : (89, 83, 63)
void rootAna_acceptance_sglmutest(int mrapnpt=89, bool isPrompt=true){
	...
	else {cout << "select among MrapNpt = 89, 83, 63"<< endl; return; }
	...
	for (int ipt=0; ipt<nPtBinsTmp; ipt++){
		...
		else {ptBinsArr[ipt]=ptBinsArr3pt[ipt];}
	}
  ...
  Double_t rapBinsArr6rap[7] = {-2.4, -1.97, -1.37, -0.47, 0.43, 1.03, 1.46};
	...
	TH2D *h2D_Den_pt_y_Pbp_fine = new TH2D("h2D_Den_pt_y_Pbp_fine","",100,-2.5,2.5,100,0,5);
	TH2D *h2D_Num_pt_y_Pbp_fine = new TH2D("h2D_Num_pt_y_Pbp_fine","",100,-2.5,2.5,100,0,5);
	TH2D *h2D_Acc_pt_y_Pbp_fine = new TH2D("h2D_Acc_pt_y_Pbp_fine","",100,-2.5,2.5,100,0,5);
...
//			if (mrapnpt==89) h2D_Den_pt_y_Pbp_fine->Fill(dkid0_eta,dkid0_pt);
			if (mrapnpt==89) h2D_Den_pt_y_Pbp_fine->Fill(dkid1_eta,dkid1_pt);
...
//				if (mrapnpt==89) h2D_Num_pt_y_Pbp_fine->Fill(dkid0_eta,dkid0_pt);
				if (mrapnpt==89) h2D_Num_pt_y_Pbp_fine->Fill(dkid1_eta,dkid1_pt);
			}
		}
	}// end of loop
	...
	////////////////////////////////////////////////////////////////////////////
	// Save the data!
	TFile *outFile = new TFile(Form("AccAna_%s_sglmutest.root",strName),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
/*
