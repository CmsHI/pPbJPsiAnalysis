...
TFile* fTnpRate = new TFile("../Efficiency/tagAndProbe/TnPDataVsMc_oldcut_oldeta_private_lowerptlimit.root");
...
/////// main func. ///////
int rootAna_efficiency_8rap3pt_check5_oldacc_oldeta_private_lowerptlimit_eta0912(char *strBinning = "8rap3pt", bool isPrompt = true, bool is1st = true, bool useCtErrCut =true, bool useSF=true, bool useZvtxWeight=true){
  ...
  mytree->SetBranchAddress("Gen_QQ_mumi_4mom", &Gen_QQ_mumi_4mom, &b_Gen_QQ_mumi_4mom);
	...
	// --- save as a root file
	TFile *outFile = new TFile(Form("EffAna_%s_CtErr%d_SF%d_Zvtx%d_check5_oldacc_oldeta_private_lowerptlimit_eta0912.root",strName, (int)useCtErrCut ,(int)useSF, (int)useZvtxWeight),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
	h2D_ctErrmin->Write();
	h2D_ctErrmax->Write();
...
*/
	return (
				...
				(1.2 <= TMath::Abs(muEta) && TMath::Abs(muEta) < 2.1 && muPt >= -(1.0/0.9)*TMath::Abs(muEta)+(1.2*(1.0/0.9)+2.6)) ||
				(2.1 <= TMath::Abs(muEta) && TMath::Abs(muEta) < 2.4 && muPt >= 1.3) );
}
