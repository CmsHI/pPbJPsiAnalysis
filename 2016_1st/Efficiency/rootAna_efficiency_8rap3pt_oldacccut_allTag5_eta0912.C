...
TFile* fTnpRate = new TFile("../Efficiency/tagAndProbe/TnPDataVsMc_newcut_neweta_officialTag5_lowerptlimit.root");
...
/////// main func. ///////
int rootAna_efficiency_8rap3pt_oldacccut_allTag5_eta0912(char *strBinning = "8rap3pt", bool isPrompt = true, bool is1st = true, bool useCtErrCut =true, bool useSF=false, bool useZvtxWeight=true){
  ...
  mytree->SetBranchAddress("Gen_QQ_mumi_4mom", &Gen_QQ_mumi_4mom, &b_Gen_QQ_mumi_4mom);
	...
	// --- save as a root file
	TFile *outFile = new TFile(Form("EffAna_%s_CtErr%d_SF%d_Zvtx%d_oldacccut_allTag5_eta0912.root",strName, (int)useCtErrCut ,(int)useSF, (int)useZvtxWeight),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
	h2D_ctErrmin->Write();
	h2D_ctErrmax->Write();
...
bool kineCut(double muPt, double muEta, double muP) {
	...
	*/
	return ( 
				(TMath::Abs(muEta) < 0.9 && muPt >=3.3) ||
				(1.2 <= TMath::Abs(muEta) &&  TMath::Abs(muEta) < 1.3 && muPt >=3.3) ||
				(1.3 <= TMath::Abs(muEta) && TMath::Abs(muEta) < 2.2 && muP >=2.9) ||
				(2.2 <= TMath::Abs(muEta) && TMath::Abs(muEta) < 2.4 && muPt >= 0.8) );
}
