...
/////// main func. ///////
int rootAna_efficiency_8rap3pt_oldacccut_check5(char *strBinning = "8rap3pt", bool isPrompt = true, bool is1st = true, bool useCtErrCut =true, bool useSF=false, bool useZvtxWeight=true){
  ...
  mytree->SetBranchAddress("Gen_QQ_mumi_4mom", &Gen_QQ_mumi_4mom, &b_Gen_QQ_mumi_4mom);
	...
	// --- save as a root file
	TFile *outFile = new TFile(Form("EffAna_%s_CtErr%d_SF%d_Zvtx%d_oldacccut_check5.root",strName, (int)useCtErrCut ,(int)useSF, (int)useZvtxWeight),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
	h2D_ctErrmin->Write();
	h2D_ctErrmax->Write();
