...
HiOnia2MuMuPAT::HiOnia2MuMuPAT(const edm::ParameterSet& iConfig):
  ...
  dimuonMassMin_(iConfig.existsAs<double>("dimuonMassMin") ? iConfig.getParameter<double>("dimuonMassMin") : 0), // KYO
	dimuonMassMax_(iConfig.existsAs<double>("dimuonMassMax") ? iConfig.getParameter<double>("dimuonMassMax") : 200), // KYO 
...
{  
  ...
  for(View<pat::Muon>::const_iterator it = muons->begin(), itend = muons->end(); it != itend; ++it){
    ...
    for(View<pat::Muon>::const_iterator it2 = it+1; it2 != itend;++it2){
      ...
      // ---- apply dimuon mass range cut ---- // KYO
			if (dimuonMassMin_ >= jpsi.M() || dimuonMassMax_ <= jpsi.M() ) continue;
	...
	if (genMu1.isNonnull() && genMu2.isNonnull()) {
	  if (genMu1->numberOfMothers()>0 && genMu2->numberOfMothers()>0){
	    ...
	    iEvent.getByLabel("genParticles", theGenParticles); // KYO pp
//	    iEvent.getByLabel("hiGenParticles", theGenParticles); // KYO embedded or data?
