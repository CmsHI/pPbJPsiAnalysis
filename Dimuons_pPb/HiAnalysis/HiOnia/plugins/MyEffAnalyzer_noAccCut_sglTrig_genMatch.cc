...
class MyEffAnalyzer_noAccCut_sglTrig_genMatch : public edm::EDAnalyzer {
public:
  explicit MyEffAnalyzer_noAccCut_sglTrig_genMatch(const edm::ParameterSet&);
  ~MyEffAnalyzer_noAccCut_sglTrig_genMatch();
...
private:
  ...
  bool checkCuts(const pat::CompositeCandidate* cand, const pat::Muon* muon1,  const pat::Muon* muon2, bool(MyEffAnalyzer_noAccCut_sglTrig_genMatch::* callFunc1)(const pat::Muon*), bool(MyEffAnalyzer_noAccCut_sglTrig_genMatch::* callFunc2)(const pat::Muon*)); 
...
//
MyEffAnalyzer_noAccCut_sglTrig_genMatch::MyEffAnalyzer_noAccCut_sglTrig_genMatch(const edm::ParameterSet& iConfig):
...
MyEffAnalyzer_noAccCut_sglTrig_genMatch::~MyEffAnalyzer_noAccCut_sglTrig_genMatch()
{
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //   using namespace edm;
  InitEvent();
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillRecoHistos(int lastSign) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillTreeMuon(const pat::Muon* muon, int iType, int trigBits) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillTreeJpsi(int iSign, int count) {
...
*/
  ...
  if (_muonLessPrimaryVertex && aJpsiCand->hasUserData("muonlessPV")) {
    ...
    cout << "MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillTreeJpsi: no PVfor muon pair stored" << endl;
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillRecoJpsi(int iSign, int count, std::string trigName, std::string centName) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillHistosAndDS(unsigned int theCat, const pat::CompositeCandidate* aJpsiCand) {

  return;
}

void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::checkTriggers(const pat::CompositeCandidate* aJpsiCand) {
  const pat::Muon* muon1 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon1"));
  const pat::Muon* muon2 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon2"));
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::makeCuts(int sign) {
  ...
  if (collJpsi.isValid()) {
	...
	std::cout << "MyEffAnalyzer_noAccCut_sglTrig_genMatch::makeCuts: no PV for muon pair stored" << std::endl;
	...
	/*
	// global + global?
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selGlobalMuon)){
	  _thePassedCats[sign].push_back(GlbGlb);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
		
	// global + tracker? (x2)    
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}

	if (checkCuts(cand,muon2,muon1,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
	*/
	...
	// tracker + tracker?  
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selTrackerMuon,&MyEffAnalyzer_noAccCut_sglTrig_genMatch::selTrackerMuon)){
...
bool
MyEffAnalyzer_noAccCut_sglTrig_genMatch::checkCuts(const pat::CompositeCandidate* cand, const pat::Muon* muon1,  const pat::Muon* muon2, bool(MyEffAnalyzer_noAccCut_sglTrig_genMatch::* callFunc1)(const pat::Muon*), bool(MyEffAnalyzer_noAccCut_sglTrig_genMatch::* callFunc2)(const pat::Muon*)) {
...
MyEffAnalyzer_noAccCut_sglTrig_genMatch::theBestQQ(int sign) {
...
bool
MyEffAnalyzer_noAccCut_sglTrig_genMatch::isMuonInAccept(const pat::Muon* aMuon) {
...
bool
MyEffAnalyzer_noAccCut_sglTrig_genMatch::selGlobalMuon(const pat::Muon* aMuon) {
...
bool 
MyEffAnalyzer_noAccCut_sglTrig_genMatch::selTrackerMuon(const pat::Muon* aMuon) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::InitEvent()
{
  for (unsigned int iTr=1;iTr<NTRIGGERS;++iTr) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillGenInfo() // KYO - diff. with HiOnia (gamma radiation included)
{
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillRecoTracks()
{
  if (collTracks.isValid()) {
    for(std::vector<reco::Track>::const_iterator it=collTracks->begin();
	it!=collTracks->end(); ++it) {
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::fillRecoMuons(int iCent)
{
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::InitTree()
{
...
void 
MyEffAnalyzer_noAccCut_sglTrig_genMatch::beginJob()
{
...
void 
MyEffAnalyzer_noAccCut_sglTrig_genMatch::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
...
void 
MyEffAnalyzer_noAccCut_sglTrig_genMatch::endJob() {
...
TLorentzVector
MyEffAnalyzer_noAccCut_sglTrig_genMatch::lorentzMomentum(const reco::Candidate::LorentzVector& p) {
  TLorentzVector res;
...
void
MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport(const edm::Event &iEvent ,const edm::EventSetup& iSetup)
{
  ...
  if ( hltConfigInit ) {
    ...
    for (std::map<std::string, unsigned int>::iterator it = mapTriggernameToHLTbit.begin(); it != mapTriggernameToHLTbit.end(); it++) {
      ...
      if (triggerIndex >= n) {
	//	std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- TriggerName " << it->first << " not available in config!" << std::endl;
      }
      else {
	it->second= triggerIndex;
	//	std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- TriggerName " << it->first << " available in config!" << std::endl;
      }
    }
  }
  ...
  try {
    ...
    //    std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- J/psi TriggerResult is present in current event" << std::endl;
  }
  catch(...) {
    //    std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- J/psi TriggerResults NOT present in current event" << std::endl;
  }
  if ( collTriggerResults.isValid() ){
    //    std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- J/psi TriggerResults IS valid in current event" << std::endl;
  ...
  } else std::cout << "[MyEffAnalyzer_noAccCut_sglTrig_genMatch::hltReport] --- TriggerResults NOT valid in current event" << std::endl;

  return;
}

//define this as a plug-in
DEFINE_FWK_MODULE(MyEffAnalyzer_noAccCut_sglTrig_genMatch);
