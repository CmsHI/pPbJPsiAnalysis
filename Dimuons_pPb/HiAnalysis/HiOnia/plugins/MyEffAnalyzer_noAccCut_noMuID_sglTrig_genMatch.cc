...
class MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch : public edm::EDAnalyzer {
public:
  explicit MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch(const edm::ParameterSet&);
  ~MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch();
...
private:
  ...
  bool checkCuts(const pat::CompositeCandidate* cand, const pat::Muon* muon1,  const pat::Muon* muon2, bool(MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::* callFunc1)(const pat::Muon*), bool(MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::* callFunc2)(const pat::Muon*)); 
  ...
  TTree* myTree;
	...
	//TLorentzVector dummy_4mom; // dummy value
  ...
  TClonesArray* Reco_QQ_mupl_matchedGen_4mom; //KYO - "GEN" matched to Reco_QQ_mupl
  TClonesArray* Reco_QQ_mumi_matchedGen_4mom; //KYO - "GEN" matched to Reco_QQ_mumi
  ...
  int Reco_QQ_mupl_trig[Max_QQ_size];      // Vector of trigger bits matched to the plus muon
  int Reco_QQ_mumi_trig[Max_QQ_size];      // Vector of trigger bits matched to the minus muon
  
	float Reco_QQ_mupl_matchedGen_dR[Max_QQ_size];      // deltaR b/w GEN-RECO matching for the plus muon
  float Reco_QQ_mumi_matchedGen_dR[Max_QQ_size];      // deltaR b/w GEN-RECO matching for the minus muon
  ...
  bool Reco_QQ_mumi_TMOneStaTight[Max_QQ_size]; // Vector of TMOneStationTight for minus muon
	...
	bool Reco_QQ_mupl_isHighPurity[Max_QQ_size];  // Track high-purity flag for plus inner track  muons //KYO
	bool Reco_QQ_mumi_isHighPurity[Max_QQ_size];  // Track high-purity flag for minus inner track muons //KYO
  ...
  bool Reco_mu_isHighPurity[Max_mu_size];  // Track high-purity flag for inner track muons //KYO
 ...
 // Triger stuff
  ...
  bool isTriggerMatchedMu1[sNTRIGGERS]; // KYO
  bool isTriggerMatchedMu2[sNTRIGGERS]; // KYO
...
//
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch(const edm::ParameterSet& iConfig):
  ...
  _doGenMatching(iConfig.getUntrackedParameter<bool>("doGenMatching",false) ),
  ...
  _iConfig(iConfig)
{
  ...
  for (unsigned int iTr = 1; iTr<NTRIGGERS; ++iTr) {
    ...
    isTriggerMatchedMu1[iTr] = false; // KYO
    isTriggerMatchedMu2[iTr] = false; // KYO
...
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::~MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch()
{
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //   using namespace edm;
  InitEvent();
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillRecoHistos(int lastSign) {
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillTreeMuon(const pat::Muon* muon, int iType, int trigBits) {
  ...
  if (!_theMinimumFlag) {
//    Reco_mu_isHighPurity[Reco_mu_size] = iTrack->quality(reco::TrackBase::qualityByName("highPurity")); //KYO    
		Reco_mu_isHighPurity[Reco_mu_size] = iTrack->quality(reco::TrackBase::highPurity); //KYO
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillTreeJpsi(int iSign, int count) {
...
*/
  ...
  for (unsigned int iTr=1; iTr<NTRIGGERS; ++iTr) {
    if (isTriggerMatched[iTr])
      trigBits += pow(2,iTr-1);
    if (isTriggerMatchedMu1[iTr])
      trigBitsMu1 += pow(2,iTr-1);
    if (isTriggerMatchedMu2[iTr])
      trigBitsMu2 += pow(2,iTr-1);
  }
  ...
  if (_muonLessPrimaryVertex && aJpsiCand->hasUserData("muonlessPV")) {
    ...
    cout << "MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillTreeJpsi: no PVfor muon pair stored" << endl;
  ...
  reco::TrackRef gTrack_mumi;
	...
	// KYO gen matching
	if (_doGenMatching) {
		TLorentzVector vGenmupl;
		TLorentzVector vGenmumi;
		float tmp_mupl_min_dR = 531.;
		...
		if (collGenParticles.isValid()) {
			...
			it!=collGenParticles->end();++it) {
				...
				gen->numberOfDaughters() >= 2) {
					unsigned int numDs = gen->numberOfDaughters();
					//std::cout << "numDs = " <<numDs << endl;
					const reco::Candidate* genMuons[numDs];
					for (unsigned int i=0; i<numDs; i++) {
						genMuons[i] = gen->daughter(i);
						if ( abs(genMuons[i]->pdgId()) == 13 && genMuons[i]->status() == 1){
							/*
							cout << i<<"th numDs !!!! "<<endl;
							cout << "muon1 charge = " << muon1->charge() << endl;
							cout << "muon2 charge = " << muon2->charge() << endl;
							cout << "genMuons[i] charge = " << genMuons[i]->charge() << endl;
							cout << "muon1 eta = " << muon1->eta() << endl;
							cout << "muon2 eta = " << muon2->eta() << endl;
							cout << "genMuons[i] eta = " << genMuons[i]->eta() << endl;
							cout << "muon1 phi = " << muon1->phi() << endl;
							cout << "muon2 phi = " << muon2->phi() << endl;
							cout << "genMuons[i] phi = " << genMuons[i]->phi() << endl;
  						*/
							if (muon1->charge() > muon2->charge()) { // muon1 = mupl
								if (muon1->charge() == genMuons[i]->charge() ){
									//cout << "muon1 & genMuons[i] = plus!" << endl;
									tmp_mupl_dR = deltaR(muon1->eta(),muon1->phi(),genMuons[i]->eta(),genMuons[i]->phi());
									if ( tmp_mupl_dR < tmp_mupl_min_dR) { //find shortest dR
										tmp_mupl_min_dR=tmp_mupl_dR;
										vGenmupl = lorentzMomentum(genMuons[i]->p4());
									}
								}
								else { 
									//cout << "muon2 & genMuons[i] = minus!" << endl;
									tmp_mumi_dR = deltaR(muon2->eta(),muon2->phi(),genMuons[i]->eta(),genMuons[i]->phi());
									if ( tmp_mumi_dR < tmp_mumi_min_dR) {
										tmp_mumi_min_dR=tmp_mumi_dR;
										vGenmumi = lorentzMomentum(genMuons[i]->p4());
									}
								}
							}
							else { // muon2 = mupl
								if (muon1->charge() == genMuons[i]->charge() ){
									//cout << "muon1 & genMuons[i] = minus!" << endl;
									tmp_mumi_dR = deltaR(muon1->eta(),muon1->phi(),genMuons[i]->eta(),genMuons[i]->phi());
									if ( tmp_mumi_dR < tmp_mumi_min_dR) {
										tmp_mumi_min_dR=tmp_mumi_dR;
										vGenmumi = lorentzMomentum(genMuons[i]->p4());
									}
									...
									//cout << "muon2 & genMuons[i] = plus!" << endl;
									tmp_mupl_dR = deltaR(muon2->eta(),muon2->phi(),genMuons[i]->eta(),genMuons[i]->phi());
									if ( tmp_mupl_dR < tmp_mupl_min_dR) {
										tmp_mupl_min_dR=tmp_mupl_dR;
										vGenmupl = lorentzMomentum(genMuons[i]->p4());
									}
								}
							}
						} //end of muon condition 13
					} // end of loop for daughters (numDs)
				} /// end of Jpsi condition 443
     	} // iterator for loop 
		} //valid
		Reco_QQ_mupl_matchedGen_dR[Reco_QQ_size] = tmp_mupl_min_dR;
		Reco_QQ_mumi_matchedGen_dR[Reco_QQ_size] = tmp_mumi_min_dR;
	  new((*Reco_QQ_mupl_matchedGen_4mom)[Reco_QQ_size])TLorentzVector(vGenmupl);
	  new((*Reco_QQ_mumi_matchedGen_4mom)[Reco_QQ_size])TLorentzVector(vGenmumi);
		//std::cout << "Reco_QQ_size =" << Reco_QQ_size << endl;
		//std::cout << "Reco_QQ_mupl_matchedGen_dR[Reco_QQ_size] = "<<Reco_QQ_mupl_matchedGen_dR[Reco_QQ_size] << endl;
		//std::cout << "Reco_QQ_mumi_matchedGen_dR[Reco_QQ_size] = "<<Reco_QQ_mumi_matchedGen_dR[Reco_QQ_size] << endl;
	}
  ...
  if (muon1->charge() > muon2->charge()) {
    ...
    new((*Reco_QQ_mumi_4mom)[Reco_QQ_size])TLorentzVector(vMuon2);
  	Reco_QQ_mupl_trig[Reco_QQ_size] = trigBitsMu1;
  	Reco_QQ_mumi_trig[Reco_QQ_size] = trigBitsMu2;
    ...
    new((*Reco_QQ_mumi_4mom)[Reco_QQ_size])TLorentzVector(vMuon1);
		...
		Reco_QQ_mupl_trig[Reco_QQ_size] = trigBitsMu2;
  	Reco_QQ_mumi_trig[Reco_QQ_size] = trigBitsMu1;
  ...
  if (!_theMinimumFlag) {
    Reco_QQ_mupl_isHighPurity[Reco_QQ_size] = iTrack_mupl->quality(reco::TrackBase::highPurity); //KYO
		Reco_QQ_mumi_isHighPurity[Reco_QQ_size] = iTrack_mumi->quality(reco::TrackBase::highPurity); //KYO
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillRecoJpsi(int iSign, int count, std::string trigName, std::string centName) {
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillHistosAndDS(unsigned int theCat, const pat::CompositeCandidate* aJpsiCand) {

  return;
}

void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::checkTriggers(const pat::CompositeCandidate* aJpsiCand) {
  const pat::Muon* muon1 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon1"));
  const pat::Muon* muon2 = dynamic_cast<const pat::Muon*>(aJpsiCand->daughter("muon2"));
    ...
    */
    ...
    if (iTr > NTRIGGERS_DBL) {  // single triggers here
      ...
      isTriggerMatched[iTr] = pass1 && pass2;
		...
		isTriggerMatchedMu1[iTr] = pass1;
		isTriggerMatchedMu2[iTr] = pass2;
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::makeCuts(int sign) {
  ...
  if (collJpsi.isValid()) {
	...
	std::cout << "MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::makeCuts: no PV for muon pair stored" << std::endl;
	...
	/*
	// global + global?
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selGlobalMuon)){
	  _thePassedCats[sign].push_back(GlbGlb);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
		
	// global + tracker? (x2)    
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}

	if (checkCuts(cand,muon2,muon1,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selGlobalMuon,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selTrackerMuon)){
	  _thePassedCats[sign].push_back(GlbTrk);  _thePassedCands[sign].push_back(cand);
	  //	  continue;
	}
	*/
	...
	// tracker + tracker?  
	if (checkCuts(cand,muon1,muon2,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selTrackerMuon,&MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selTrackerMuon)){
...
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::theBestQQ(int sign) {
...
bool
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::isMuonInAccept(const pat::Muon* aMuon) {
...
bool
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selGlobalMuon(const pat::Muon* aMuon) {
...
bool 
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::selTrackerMuon(const pat::Muon* aMuon) {
  ...
  if(!_applycuts)
    return true;
// below 2 are not needed for noMuID
//  reco::TrackRef iTrack = aMuon->innerTrack();
//  const reco::HitPattern& p = iTrack->hitPattern();
	////const reco::TrackBase::TrackQuality _trackHighPurity = reco::TrackBase::qualityByName("highPurity"); // KYO
/*
  return (isMuonInAccept(aMuon) &&
...
*/
	// Jpsi new soft muon ID since August 2014 (KYO)
/*
  return (isMuonInAccept(aMuon) &&
          //iTrack->quality(_trackHighPurity) && //KYO
    			iTrack->quality(reco::TrackBase::highPurity) && //KYO
					aMuon->muonID("TrackerMuonArbitrated") &&
          aMuon->muonID("TMOneStationTight") &&
          p.trackerLayersWithMeasurement() > 5 &&
          p.pixelLayersWithMeasurement() > 0 &&
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::InitEvent()
{
  for (unsigned int iTr=1;iTr<NTRIGGERS;++iTr) {
  ...
  Reco_QQ_mupl_matchedGen_4mom->Clear(); //KYO
  Reco_QQ_mumi_matchedGen_4mom->Clear(); //KYO
  Reco_mu_4mom->Clear();
  Reco_mu_3vec->Clear();
  Reco_trk_4mom->Clear();
  Reco_trk_vtx->Clear();
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillGenInfo() // KYO - diff. with HiOnia (gamma radiation included)
{
	...
	if (collGenParticles.isValid()) {
    	...
    	if (abs(gen->pdgId()) == _oniaPDG  && gen->status() == 2 &&
			gen->numberOfDaughters() >= 2) {
				unsigned int numD = gen->numberOfDaughters();
				//std::cout << "numD = " << numD << std::endl;
				const reco::Candidate* genMuons[numD];
				//const reco::Candidate* genMuon1 = gen->daughter(0);
				//const reco::Candidate* genMuon2 = gen->daughter(1);
				for (unsigned int i=0; i<numD; i++) {
					genMuons[i] = gen->daughter(i);
				}
				for (unsigned int ione=0; ione <numD-1; ione++) {
					for (unsigned int itwo=ione+1; itwo <numD; itwo++) {
						const reco::Candidate* genMuon1 = genMuons[ione];
						const reco::Candidate* genMuon2 = genMuons[itwo];
	    			...
	    			genMuon2->status() == 1 ) {
	  					...
	  					Gen_QQ_type[Gen_QQ_size] = _isPromptMC ? 0 : 1; // prompt: 0, non-prompt: 1
							...
							Gen_QQ_size++;
						} // end of mupl mumi condition
					}
				}
			} /// end of Jpsi condition
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillRecoTracks()
{
  if (collTracks.isValid()) {
    for(std::vector<reco::Track>::const_iterator it=collTracks->begin();
	it!=collTracks->end(); ++it) {
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::fillRecoMuons(int iCent)
{
  int nL1DoubleMuOpenMuons=0;
  int nL1DoubleMuOpenHighQMuons=0; // KYO
  int nL2DoubleMu3Muons=0;
  int nL3Mu3Muons=0; // KYO
  int nL3Mu7Muons=0;  // KYO
  int nL3Mu12Muons=0;
  int nGoodMuons=0;
  ...
  if (collMuon.isValid()) {
	...
	if (_combineCategories) {
      ...
      muType = 0;
			if (muon->isGlobalMuon() && selGlobalMuon(muon)) muType = muType+pow(2,0);
			if (muon->isTrackerMuon() && selTrackerMuon(muon)) muType = muType+pow(2,1);
  ...
  //          muType==1 
	...
	for (unsigned int iTr=1; iTr<NTRIGGERS; ++iTr) {
	  ...
	  if ( muHLTMatchesFilter.size() > 0 ) {
	    ...
	    if (iTr==2) nL1DoubleMuOpenHighQMuons++;
	    if (iTr==3) nL2DoubleMu3Muons++;
	    //if (iTr==6) nL3Mu12Muons++; // KYO bug???
	    if (iTr==5) nL3Mu3Muons++;
	    if (iTr==6) nL3Mu7Muons++;
	    if (iTr==7) nL3Mu12Muons++; 
	  }
	}
	if (_fillTree)
  ...
  hL1DoubleMuOpenHighQ->Fill(nL1DoubleMuOpenHighQMuons);
  hL2DoubleMu3->Fill(nL2DoubleMu3Muons);
  hL3Mu3->Fill(nL3Mu3Muons);
  hL3Mu7->Fill(nL3Mu7Muons);
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::InitTree()
{
  ...
  Reco_QQ_mupl_matchedGen_4mom = new TClonesArray("TLorentzVector",10);
  Reco_QQ_mumi_matchedGen_4mom = new TClonesArray("TLorentzVector",10);
  ...
  myTree->Branch("Reco_QQ_mupl_matchedGen_4mom", "TClonesArray", &Reco_QQ_mupl_matchedGen_4mom, 32000, 0);
  myTree->Branch("Reco_QQ_mumi_matchedGen_4mom", "TClonesArray", &Reco_QQ_mumi_matchedGen_4mom, 32000, 0);
  ...
  myTree->Branch("Reco_QQ_NtrkDeltaR05", Reco_QQ_NtrkDeltaR05, "Reco_QQ_NtrkDeltaR05[Reco_QQ_size]/I");
	...
	myTree->Branch("Reco_QQ_mupl_trig", Reco_QQ_mupl_trig,   "Reco_QQ_mupl_trig[Reco_QQ_size]/I");//KYO
	myTree->Branch("Reco_QQ_mumi_trig", Reco_QQ_mumi_trig,   "Reco_QQ_mumi_trig[Reco_QQ_size]/I"); //KYO
	myTree->Branch("Reco_QQ_mupl_matchedGen_dR", Reco_QQ_mupl_matchedGen_dR,   "Reco_QQ_mupl_matchedGen_dR[Reco_QQ_size]/F");//KYO
	myTree->Branch("Reco_QQ_mumi_matchedGen_dR", Reco_QQ_mumi_matchedGen_dR,   "Reco_QQ_mumi_matchedGen_dR[Reco_QQ_size]/F"); //KYO

  if (!_theMinimumFlag) {
		myTree->Branch("Reco_QQ_mupl_isHighPurity", Reco_QQ_mupl_isHighPurity,   "Reco_QQ_mupl_isHighPurity[Reco_QQ_size]/O"); //KYO
		myTree->Branch("Reco_QQ_mumi_isHighPurity", Reco_QQ_mumi_isHighPurity,   "Reco_QQ_mumi_isHighPurity[Reco_QQ_size]/O"); //KYO
  ...
  if (!_theMinimumFlag) {
		myTree->Branch("Reco_mu_isHighPurity", Reco_mu_isHighPurity,   "Reco_mu_isHighPurity[Reco_mu_size]/O"); // KYO
...
void 
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::beginJob()
{
  ...
  hL1DoubleMuOpenHighQ = new TH1F("hL1DoubleMuOpenHighQ","hL1DoubleMuOpenHighQ",10,0,10);
  hL2DoubleMu3    = new TH1F("hL1DoubleMu3","hL2DoubleMu3",10,0,10);
  hL3Mu3         = new TH1F("hL3Mu3","hL3Mu3",10,0,10);
  hL3Mu7         = new TH1F("hL3Mu7","hL3Mu7",10,0,10);
  hL3Mu12         = new TH1F("hL3Mu12","hL3Mu12",10,0,10);
  
  hGoodMuonsNoTrig->Sumw2();
  hGoodMuons->Sumw2();
  hL1DoubleMuOpen->Sumw2();
  hL1DoubleMuOpenHighQ->Sumw2();
  hL2DoubleMu3->Sumw2();
  hL3Mu3->Sumw2();
  hL3Mu7->Sumw2();
  hL3Mu12->Sumw2();
...
void 
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
...
void 
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::endJob() {
...
TLorentzVector
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::lorentzMomentum(const reco::Candidate::LorentzVector& p) {
  TLorentzVector res;
...
void
MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport(const edm::Event &iEvent ,const edm::EventSetup& iSetup)
{
  ...
  if ( hltConfigInit ) {
    ...
    for (std::map<std::string, unsigned int>::iterator it = mapTriggernameToHLTbit.begin(); it != mapTriggernameToHLTbit.end(); it++) {
      ...
      if (triggerIndex >= n) {
	//	std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- TriggerName " << it->first << " not available in config!" << std::endl;
      }
      else {
	it->second= triggerIndex;
	//	std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- TriggerName " << it->first << " available in config!" << std::endl;
      }
    }
  }
  ...
  try {
    ...
    //    std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- J/psi TriggerResult is present in current event" << std::endl;
  }
  catch(...) {
    //    std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- J/psi TriggerResults NOT present in current event" << std::endl;
  }
  if ( collTriggerResults.isValid() ){
    //    std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- J/psi TriggerResults IS valid in current event" << std::endl;
  ...
  } else std::cout << "[MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch::hltReport] --- TriggerResults NOT valid in current event" << std::endl;

  return;
}

//define this as a plug-in
DEFINE_FWK_MODULE(MyEffAnalyzer_noAccCut_noMuID_sglTrig_genMatch);
