closureTest()
{

  TFile * fcorr = new TFile("../corrections/trackCorrections_HIN12017v1_HijingCombined.root");
  char corrdir[256] = "trkCorr_HIN12017";
  //TFile * fmc = new TFile("trackAnalysis_HIN12017v1_HijingJet170closure.root");
  TFile * fmc = new TFile("trackAnalysis_HIN12017v1_HijingMBclosure.root");

  double etaMin = 1.94;
  double etaMax = 2.34;

  TH3F *hSim = (TH3F*) fcorr->Get(Form("%s/hsim3D",corrdir));
  TH3F *hFak = (TH3F*) fcorr->Get(Form("%s/hfak3D",corrdir));
  TH3F *hMul = (TH3F*) fcorr->Get(Form("%s/hmul3D",corrdir));
  TH3F *hSec = (TH3F*) fcorr->Get(Form("%s/hsec3D",corrdir));
  TH3F *hEff = (TH3F*) fcorr->Get(Form("%s/heff3D",corrdir));
  TH3F *hRec = (TH3F*) fcorr->Get(Form("%s/hrec3D",corrdir));

  TH3F *hGen = (TH3F*) fmc->Get("trkAna_HIN12017_MC/genSpectrum");
  TH3F *hUncorrTrk = (TH3F*) fmc->Get("trkAna_HIN12017_MC/trkSpectrum");
  hUncorrTrk->SetName("trkSpectrumUncorr");
  TH3F *hTrk = (TH3F*) fmc->Get("trkAna_HIN12017_MC/trkSpectrum");
 // TH3F * hTrk = hUncorrTrk->Clone("correctedTrk");

  TF3 * unity = new TF3("unity","1.0+0.0*x*y*z",-1000.,1000.,-1000.,1000.,-1000.,1000.);

  TH1F * hnevt = fmc->Get("trkAna_HIN12017_MC/events");
  double nevt = hnevt->Integral();

  // build and apply corrections

  hFak->Divide(hRec);
  hFak->Multiply(unity,-1.0);
  hFak->Add(unity,1.0);

  hSec->Divide(hRec);
  hSec->Multiply(unity,-1.0);
  hSec->Add(unity,1.0);

  hEff->Divide(hSim);

  hMul->Divide(hSim);
  hMul->Add(unity,1.0);

  hTrk->Divide(hEff);
  hTrk->Divide(hMul);
  hTrk->Multiply(hFak);
  hTrk->Multiply(hSec);

  // create 1D histos

  TH2F * chTrk = (TH2F*) hTrk->Project3D("yxe");
  TH2F * chGen = (TH2F*) hGen->Project3D("yxe");
  TH2F * chUncorrTrk = (TH2F*) hUncorrTrk->Project3D("yxe");

  Int_t etabin10m=chTrk->GetXaxis()->FindBin(etaMin+0.01);
  Int_t etabin10p=chTrk->GetXaxis()->FindBin(etaMax-0.01);


  TH1D * hTrkPt = (TH1D *) chTrk->ProjectionY("hTrkPt",etabin10m,etabin10p,"e");
  TH1D * hGenPt = (TH1D *) chGen->ProjectionY("hGenPt",etabin10m,etabin10p,"e");
  TH1D * hUncorrTrkPt = (TH1D *) chUncorrTrk->ProjectionY("hUncorrTrkPt",etabin10m,etabin10p,"e");

  double etaWid = etaMax - etaMin;

  for(int i = 1; i <= hTrkPt->GetNbinsX(); i++) 
  {
    double binWid = hTrkPt->GetBinWidth(i);
    hTrkPt->SetBinContent(i, hTrkPt->GetBinContent(i)/binWid/etaWid/nevt);
    hTrkPt->SetBinError(i, hTrkPt->GetBinError(i)/binWid/etaWid/nevt);
    hUncorrTrkPt->SetBinContent(i, hUncorrTrkPt->GetBinContent(i)/binWid/etaWid/nevt);
    hUncorrTrkPt->SetBinError(i, hUncorrTrkPt->GetBinError(i)/binWid/etaWid/nevt);
    hGenPt->SetBinContent(i, hGenPt->GetBinContent(i)/binWid/etaWid/nevt);
    hGenPt->SetBinError(i, hGenPt->GetBinError(i)/binWid/etaWid/nevt);
  }

  hTrkPt->SetMarkerStyle(20);
  hGenPt->SetMarkerStyle(20);
  hGenPt->SetMarkerColor(kRed);
  hUncorrTrkPt->SetMarkerColor(kBlue);
  hUncorrTrkPt->SetMarkerStyle(25);

   TCanvas *c1 = new TCanvas("c1", "c1",0,0,600,600);
   gStyle->SetOptFit(1);
   gStyle->SetOptStat(0);
   gStyle->SetOptTitle(0);
   c1->Range(0,0,1,1);
   c1->SetFillColor(0);
   c1->SetBorderMode(0);
   c1->SetBorderSize(2);
   //c1->SetLogy();
   c1->SetTickx(1);
   c1->SetTicky(1);
   c1->SetLeftMargin(0.16);
   c1->SetRightMargin(0.16);
   c1->SetTopMargin(0.05);
   c1->SetBottomMargin(0.13);
   c1->SetFrameFillStyle(0);
   c1->SetFrameBorderMode(0);

   c1->Divide(1,2,0,0);
   c1->cd(1);
   c1->GetPad(1)->SetLogy();
   c1->GetPad(1)->SetLogx();
   c1->GetPad(2)->SetLogx();

   TH1F * hDum = new TH1F("dum","",10,0.,20.);
   hDum->SetMinimum(1e-4);
   hDum->SetMaximum(80);
   hDum->GetYaxis()->SetTitle("Yield");

  TH1D * rTrkPt = (TH1D*) hTrkPt->Clone("ratio");
  rTrkPt->Divide(hGenPt);
 // rTrkPt->Divide(hUncorrTrkPt);

  hDum->Draw();
  hTrkPt->Draw("same");
  hGenPt->Draw("same");
  hUncorrTrkPt->Draw("same");

  TLegend * leg = new TLegend(0.2,0.2,0.5,0.4);
  leg->AddEntry(hTrkPt,"Corrected Tracks","lp");
  leg->AddEntry(hUncorrTrkPt,"Uncorrected Tracks","lp");
  leg->AddEntry(hGenPt,"Simulated Particles","lp");
  leg->Draw();

  c1->cd(2);

  TH1F * rDum = (TH1F*) hDum->Clone("rdum");
  rDum->SetMinimum(0.9);
  rDum->SetMaximum(1.1);
  rDum->GetXaxis()->SetTitle("P_{T} [GeV/c]");
  rDum->GetXaxis()->SetMoreLogLabels();
  rDum->GetYaxis()->SetTitle("Ratio Trk/Sim");
  rDum->Draw();
  rTrkPt->Draw("same");

  TLatex * tex = new TLatex(0.4,1.05,Form("%5.3f < #eta < %5.3f",etaMin,etaMax));
  tex->SetTextSize(0.043);
  tex->Draw();

}

