...
int rootAna_meanPt_rfb(char *strBin = "6rap3pt"){
	...
	Double_t ptArr[] = {5.0, 6.5, 10.0, 30.0}; // Rfb	
	...
	Double_t yArr[] = {1.93, 1.5, 0.9, 0., -0.9, -1.5, -1.93}; // Rfb
...
//				treeDatapPb->Draw(Form("Reco_QQ_4mom.Pt()>>+%s",hMeanPt[in][ir][ipt]->GetName()),totalCutpPb[in][ir][ipt],"",nEntry);
			...
			cmulti[in][ir]->Update();
			cmulti[in][ir]->SaveAs(Form("meanPt_rfb_%s_%d_%d.png",strBin,in,ir));
			//cmulti[in][ir]->Clear();
		}
	}

	/// Save as a root file
	TFile *outFile = new TFile(Form("meanPt_rfb_%s.root",strBin),"RECREATE");
