...
//	int iPos=11;//left corner
	...
	Double_t lumi_nb; Double_t lumi_nb_err;
	Double_t lumi_mub; Double_t lumi_mub_err;
	string runstring;
	string lumistring;
	...
	*/
	...
	// scaling for drawing
	double scaleF[nRap] = {1., 1., 1., 1., 1., 1., 1., 1.};
	double scaleF_true[nRap] = {100., 10., 1., 1., 10., 100., 1000., 10000.};
	if (isScale) { 
		for (int iy=0; iy<nRap; iy++){
			scaleF[iy] = scaleF_true[iy];
			cout << "scaleF = " << scaleF[iy] << endl;	
		}
	}
	...
	/// tmp
	//TFile * fSys = new TFile("../010_totalSys/TotSys_8rap9pt2gev.root");
