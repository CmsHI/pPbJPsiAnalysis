...
//  gStyle->SetPadTopMargin(0.05);
  ...
  gStyle->SetPadRightMargin(0.075);
	...
	Double_t pxtmp_lowpt[nRap]; //x point to fill tmp
	Double_t pxtmp_highpt[nRap]; //x point to fill tmp
	Double_t pytmp_lowpt[nRap]; //y point to fill tmp
	Double_t pytmp_highpt[nRap]; //y point to fill tmp
	Double_t eytmp[nRap]; //y stat error
	...
	Double_t eysysrel_pr_lowpt[nRap]; //y relative sys error
	Double_t eysysrel_pr_highpt[nRap]; //y relative sys error
	Double_t eysysrel_np_lowpt[nRap]; //y relative sys error
	Double_t eysysrel_np_highpt[nRap]; //y relative sys error
	Double_t eysys_pr_lowpt[nRap]; //y absolute error
	Double_t eysys_pr_highpt[nRap]; //y absolute error
	Double_t eysys_np_lowpt[nRap]; //y absolute error
	Double_t eysys_np_highpt[nRap]; //y absolute error
...
//	TCanvas *ctmp2 = new TCanvas("ctmp2","ctmp2",600,600);
	...
	TH1D* h1D_corrY_PR_Pbp_tmp[nPt]; // in y_lab(1st)
	TH1D* h1D_corrY_NP_Pbp_tmp[nPt]; 
	TH1D* h1D_corrY_PR_Pbp[nPt]; 
	TH1D* h1D_corrY_NP_Pbp[nPt]; 
	TH1D* h1D_corrY_PR_pPb_tmp[nPt]; // in y_lab(2nd)
	TH1D* h1D_corrY_NP_pPb_tmp[nPt]; 
	TH1D* h1D_corrY_PR_pPb[nPt]; //in y_CM
	TH1D* h1D_corrY_NP_pPb[nPt]; 
	TH1D* h1D_syserr_PR_tmp[nPt];
	TH1D* h1D_syserr_NP_tmp[nPt];
	TH1D* h1D_syserr_PR[nPt];
	TH1D* h1D_syserr_NP[nPt];
	...
	for (Int_t ipt = 0; ipt < nPt; ipt++) {
		...
		for (Int_t iy=0; iy<nRap; iy++){
			...
			h1D_corrY_PR_Pbp[ipt]->SetBinContent(nRap-iy,tmpval);
			h1D_corrY_PR_Pbp[ipt]->SetBinError(nRap-iy,tmperr);
			tmpval = h1D_corrY_NP_Pbp_tmp[ipt]->GetBinContent(iy+1);
			tmperr = h1D_corrY_NP_Pbp_tmp[ipt]->GetBinError(iy+1);
			h1D_corrY_NP_Pbp[ipt]->SetBinContent(nRap-iy,tmpval);
			h1D_corrY_NP_Pbp[ipt]->SetBinError(nRap-iy,tmperr);
			//2nd run
			...
			h1D_syserr_PR[ipt]->SetBinContent(nRap-iy,tmpval);
			h1D_syserr_PR[ipt]->SetBinError(nRap-iy,tmperr);
			tmpval = h1D_syserr_NP_tmp[ipt]->GetBinContent(iy+1);
			tmperr = h1D_syserr_NP_tmp[ipt]->GetBinError(iy+1);
			h1D_syserr_NP[ipt]->SetBinContent(nRap-iy,tmpval);
			h1D_syserr_NP[ipt]->SetBinError(nRap-iy,tmperr);
		}
	}
	...
	TH1D* h1D_corrY_PR_tot[nPt]; 
	TH1D* h1D_corrY_NP_tot[nPt]; 
	TH1D* h1D_cross_PR_tot[nPt]; 
	TH1D* h1D_cross_NP_tot[nPt]; 
	for (Int_t ipt = 0; ipt < nPt; ipt++) {
		// --- merging
		if (runCode ==0) {	
