...
void draw_1D_rawYield_pt(char* dirName = "8rap9pt2gev", bool isPrompt=true, bool isLog=false)
{
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetTitle("raw yield");
		...
		if (!isLog) {
			...
			if (isPrompt) h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetRangeUser(0.,0.24);
			else h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetRangeUser(0.,0.24);
		}
	...
	//c_multi->cd(10);//tmp
	//legUR->Draw();
	c_multi->SaveAs(Form("rawYield_%s/rawYield_pt_isPrompt%d_isLog%d.pdf",dirName,(int)isPrompt,(int)isLog));
