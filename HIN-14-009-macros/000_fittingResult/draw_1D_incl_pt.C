...
void draw_1D_incl_pt(char* dirName = "8rap9pt2gev", bool isLog=false)
{
	...
	TH2D* h2D_fit_pt_y_pPb;
		h2D_fit_pt_y_Pbp = (TH2D*)f2D->Get("otherFitInfo/h2D_fit_pt_y_nSig_Pbp");
		h2D_fit_pt_y_pPb = (TH2D*)f2D->Get("otherFitInfo/h2D_fit_pt_y_nSig_pPb");
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetTitle("inclusive yield");
		...
		if (!isLog) {
			//double maxVal = h1D_fit_pt_y_Pbp[iy]->GetMaximum();
			//h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetRangeUser(0.,maxVal*1.5);
			h1D_fit_pt_y_Pbp[iy]->GetYaxis()->SetRangeUser(0.,0.25);
		}
		h1D_fit_pt_y_Pbp[iy]->Draw("pe");
		h1D_fit_pt_y_pPb[iy]->Draw("pe same");
		if (iy==0) {
//			if (isPrompt) legUR -> SetHeader("Prompt J/#psi ");
//			else legUR -> SetHeader("Non-prompt J/#psi ");
			legUR -> AddEntry(h1D_fit_pt_y_Pbp[iy],"1st run (Pbp)","lp");
			legUR -> AddEntry(h1D_fit_pt_y_pPb[iy],"2nd run (pPb)","lp");
			legUR->Draw();
		}
...
//		c_multi->SaveAs(Form("DataMcReco_%s/data_mc_reco_Pbp_isLog%d_%d.pdf",dirName,(int)isLog,iy));
//		c_multi->Clear();
//		legUR->Clear();
	}
	c_multi->SaveAs(Form("incl_%s/incl_pt_isLog%d.pdf",dirName,(int)isLog));
