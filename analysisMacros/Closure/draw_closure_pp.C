...
int draw_closure_pp(int MrapNpt=89, int isPA =3, int accCutType = 2, bool useZvtxWeight=false, bool useSF = false)
{
  gROOT->Macro("../Style.C");
  using namespace std;
 ...
 bool isLog = false;  
  ...
  TFile * f2D = new TFile("../FittingResult/totalHist_pp_8rap9pt_newcut_Zvtx0_SF0_noPtWeight.root","READ");
  ...
  cout << "h2D_MCCorrY_PR_pp = " << h2D_MCCorrY_PR_pp << endl;
  cout << "h2D_MCCorrY_NP_pp = " << h2D_MCCorrY_NP_pp << endl;
  ...
  cout << "h2D_Acc_Den_PR_pp = " << h2D_Acc_Den_PR_pp << endl;
  cout << "h2D_Acc_Den_NP_pp = " << h2D_Acc_Den_NP_pp << endl;
  ...
  // --- prompt pp
  //TCanvas* c_PR_pp = new TCanvas("c_PR_pp","c_PR_pp",200,10,600,600);
  //c_PR_pp->cd();
  ...
  for (Int_t iy = 0; iy < nbinsX; iy++) {
    ...
    //if (isLog) h1D_corrY_PR_pp[iy]->GetYaxis()->SetRangeUser(0.01,700);
    //else h1D_corrY_PR_pp[iy]->GetYaxis()->SetRangeUser(0.0,350);
...
//    c_PR_pp->SaveAs(Form("MCClosure_%s/mc_closure_pt_PR_pp_isLog%d_%d.pdf",dirName,(int)isLog,iy));
//    c_PR_pp->Clear();
//    legUR->Clear();
  }
  
  // --- non-prompt pp
  //TCanvas* c_NP_pp = new TCanvas("c_NP_pp","c_NP_pp",200,10,600,600);
  //c_NP_pp->cd();
  ...
  for (Int_t iy = 0; iy < nbinsX; iy++) {
    ...
    //if (isLog) h1D_corrY_NP_pp[iy]->GetYaxis()->SetRangeUser(0.01,700);
    //else h1D_corrY_NP_pp[iy]->GetYaxis()->SetRangeUser(0.0,350);
    ...
    latex->DrawLatex(0.46,0.68,Form("%s",rapArr[iy].c_str()));
//    c_NP_pp->SaveAs(Form("MCClosure_%s/mc_closure_pt_NP_pp_isLog%d_%d.pdf",dirName,(int)isLog,iy));
//    c_NP_pp->Clear();
//    legUR->Clear();
  }
  ...
  for (Int_t iy = 0; iy < nbinsX; iy++) {
    ...
    hRatio_PR_pp[iy]->GetYaxis()->SetTitle("Ratio");
    ...
    if (iy==0) {
      ...
      legBL -> AddEntry(hRatio_PR_pp[iy],"[RECO/(Acc*Eff)] / GEN","lp");
      latex->DrawLatex(0.56,0.78,"[RECO/(Acc*Eff)] / GEN");
      //legBL->Draw();
    }
    latex->DrawLatex(0.56,0.88,Form("%s",rapArr[iy].c_str()));
    dashedLine(0.,1.,30.,1.,1,1);
//    c_PR_pp->SaveAs(Form("MCClosure_%s/mc_closure_ratio_PR_pp_isLog%d_%d.pdf",dirName,(int)isLog,iy));
//    c_PR_pp->Clear();
//    legBL->Clear();
  }
  ...
  for (Int_t iy = 0; iy < nbinsX; iy++) {
    ...
    hRatio_NP_pp[iy]->GetYaxis()->SetTitle("Ratio");
    ...
    //legBL -> AddEntry(hRatio_NP_pp[iy],"[RECO/(Acc*Eff)] / GEN","lp");
    //legBL->Draw();
    latex->DrawLatex(0.56,0.88,Form("%s",rapArr[iy].c_str()));
    dashedLine(0.,1.,30.,1.,1,1);
//    c_NP_pp->SaveAs(Form("MCClosure_%s/mc_closure_ratio_NP_pp_isLog%d_%d.pdf",dirName,(int)isLog,iy));
//    c_NP_pp->Clear();
//    legBL->Clear();
  }
