...
int draw_figure_mass(char *stringA = "20140301"){
	...
	for (int i=0; i<nHist; i++) {
		...
		// for mass cut
		hSigSubPbp[i]->GetXaxis()->SetRangeUser(2.9,3.3);
/*
...
//		if (i==0) hSigSubPbp[i]->SetAxisRange(0.0001,1.,"Y"); //log
		if (i==0) hSigPbp[i]->SetAxisRange(0.0001,0.2,"Y"); //linear
		...
		hSigPbp[i]->Draw("pe");
		//hSigSubPbp[i]->Draw("pe same");
		hPRMCPbp[i]->Draw("hist same");
		hNPMCPbp[i]->Draw("hist same");

		if (i==0) {
			legUL->AddEntry(hSigPbp[i],"Data","lp");
		...
		c1[i]->SaveAs(Form("VariablesSubtract/variables_log0_%s_%s.pdf",histName[i].Data(),stringA));
		c1[i]->SaveAs(Form("VariablesSubtract/variables_log0_%s_%s.png",histName[i].Data(),stringA));
		//c1[i]->Clear();
		legUR->Clear();
		legUM->Clear();
		legUL->Clear();
