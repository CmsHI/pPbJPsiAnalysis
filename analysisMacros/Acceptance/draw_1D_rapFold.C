...
void draw_1D_rapFold(TString szDir = "dir_acc_rapFold", bool isNoErr=false)
{
  gROOT->Macro("./Style.C");
  ...
  //TFile* f01 = new TFile("./dir_acc/8rap9pt_pp_NP_newcut_AccPt.root");
  TString szF01="pp_PR";
  //TString szF01="pp_NP";
  double ratiomin=0.7; 
  double ratiomax=1.3; 
  ...
  const TString szFinal = Form("%s",szF01.Data());
  ...
  for (Int_t iy = 0; iy < (double)nRap/2.; iy++) {
    //// 1)  distributions
    c1->cd(iy+1);
    SetHistStyle(h1D_01[iy],3,0);
    SetHistStyle(h1D_01[nRap-1-iy],4,10);
    ...
    latex->DrawLatex(0.53,0.19,Form("%.2f < |y_{lab}| < %.2f",rapEdge[nRap-1-iy],rapEdge[nRap-iy]));
    if (iy==0) {
...
//      legBR->AddEntry(h1D_02[iy],szF02.Data(),"lp");
  ...
  c1->SaveAs(Form("%s/comp_acc_%s.pdf",szDir.Data(),szFinal.Data()));
  c2->SaveAs(Form("%s/ratio_acc_%s.pdf",szDir.Data(),szFinal.Data()));
