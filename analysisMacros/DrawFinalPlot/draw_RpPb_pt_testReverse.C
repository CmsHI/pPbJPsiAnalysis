...
void draw_RpPb_pt_testReverse(bool sysByHand=false, bool noPtWeight=false, bool isPrompt=true)
{
  ...
  */
  ...
  for (Int_t iy = 0; iy < nRapRpPb; iy++) { 
    ...
    for (Int_t ipt = 0; ipt < nPt; ipt++) { 
      ...
      //// testReverse
      DivideValue(pytmp_pA[iy][ipt], eytmp_pA[iy][ipt], pytmp_pp[nRapRpPb-iy-1][ipt], eytmp_pp[nRapRpPb-iy-1][ipt], &pytmp[iy][ipt], &eytmp[iy][ipt]); //actual values
      DivideValue(pytmp_pA[iy][ipt], eysys_pA[iy][ipt], pytmp_pp[nRapRpPb-iy-1][ipt], eysys_pp[nRapRpPb-iy-1][ipt], &dummy1, &eysys[iy][ipt]); // syst.
...
////  globtex->DrawLatex(0.5, 0.5, "XIA");
  ...
  if (noPtWeight) { 
    c_all->SaveAs(Form("plot_RpPb_testReverse/all_RpPb_pt_isPrompt%d_noPtWeight.pdf",(int)isPrompt));
    c_all->SaveAs(Form("plot_RpPb_testReverse/all_RpPb_pt_isPrompt%d_noPtWeight.png",(int)isPrompt));
  } else {
    c_all->SaveAs(Form("plot_RpPb_testReverse/all_RpPb_pt_isPrompt%d.pdf",(int)isPrompt));
    c_all->SaveAs(Form("plot_RpPb_testReverse/all_RpPb_pt_isPrompt%d.png",(int)isPrompt));
  }
  
  ///////////////////////////////////////////////////////////////////
  //// save as a root file
  TFile *outFile;
  if (noPtWeight) outFile = new TFile(Form("plot_RpPb_testReverse/RpPb_pt_isPrompt%d_noPtWeight.root",(int)isPrompt),"RECREATE");
  else outFile = new TFile(Form("plot_RpPb_testReverse/RpPb_pt_isPrompt%d.root",(int)isPrompt),"RECREATE");
  outFile->cd();
