...
void draw_1D_RFB_pt_bhong_PbppPb(char* dirName = "8rap9pt2gev", int runCode=0, bool isZoomIn = true, bool isPrompt=false)
{
...
*/	
  ...
  gStyle->SetPadRightMargin(0.03);
	...
	cmsTextFont   = 42;  // default is helvetic-bold
	extraTextFont = 42;  // default is helvetica-italics
  ...
  Double_t ex[ntmp] = {0.,0.,0.};
	//exsys = {0.4,0.4,0.4};
	//exsys = {0.2,0.2,0.2};
	Double_t exsys[ntmp] = {0.25,0.25,0.25};
	Double_t eysysrelPR[3][ntmp] = {
	  {0.04529, 0.04795, 0.06486}, //1.5-1.03
		{0.00, 0.04245, 0.03360}, //0.9-1.5
	  {0.00, 0.04694, 0.03705} //0.0-0.9
  };
	Double_t eysysrelNP[3][ntmp] = {
	...
	// --- read-in file
  TFile * f2D = new TFile("../FittingResult/totalHist_PbppPb_8rap9pt_oldcut_nominal_Zvtx1_SF1_noPtWeight_privFit_privMC.root");
  //TFile * f2D = new TFile("../FittingResult/totalHist_PbppPb_8rap9pt_oldcut_nominal_Zvtx1_SF1_noPtWeight_privFit_privMC_PASB_PASP_zcut.root");
	...
	TH2D* h2D_CorrY_pPb;
	if (isPrompt) {
    h2D_CorrY_Pbp = (TH2D*)f2D->Get("h2D_CorrY_PR_Pbp");
    h2D_CorrY_pPb = (TH2D*)f2D->Get("h2D_CorrY_PR_pPb");
	}
  else {
    h2D_CorrY_Pbp = (TH2D*)f2D->Get("h2D_CorrY_NP_Pbp");
    h2D_CorrY_pPb = (TH2D*)f2D->Get("h2D_CorrY_NP_pPb");
  }
  h2D_CorrY_Pbp->SetName("h2D_CorrY_Pbp");	
  h2D_CorrY_pPb->SetName("h2D_CorrY_pPb");	
	...
	TH1D* h1D_CorrY[nbinsX]; 
	
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		// --- merging
		if (runCode ==0) {	
			h1D_CorrY[iy] = (TH1D*)h1D_CorrY_Pbp[iy]->Clone(Form("h1D_CorrY_%d",iy));
			h1D_CorrY[iy]->Add(h1D_CorrY_pPb[iy]);	
		}
		else if (runCode ==1) {
			h1D_CorrY[iy] = (TH1D*)h1D_CorrY_Pbp[iy]->Clone(Form("h1D_CorrY_%d",iy));
		}
		else if (runCode ==2) {
			h1D_CorrY[iy] = (TH1D*)h1D_CorrY_pPb[iy]->Clone(Form("h1D_CorrY_%d",iy));
		}
		cout <<" "<<endl;
		cout <<iy<<"th rap !!!"<<endl;
		for (int ipt=0; ipt <nbinsY; ipt ++ ){ 
			cout << ipt <<"th pt" << endl;
			cout << "h1D_CorrY = " << h1D_CorrY[iy]->GetBinContent(ipt+1)<<endl; 
		}
	}
	...
	//TLegend *legBR = new TLegend(0.49, 0.17, 0.87, 0.45); //upper left
	//TLegend *legBR = new TLegend(0.49, 0.17, 0.73, 0.44); //upper left
  ...
  globtex->SetTextFont(42);
	...
	for (int iy=0; iy<nRapRFB; iy++){
		...
		for (int ipt=0; ipt<nPtRFB; ipt++){
			...
			//abs err calcul.
			if (isPrompt) eysys[iy][ipt]=eysysrelPR[iy][ipt]*pytmp[iy][ipt];
			else eysys[iy][ipt]=eysysrelNP[iy][ipt]*pytmp[iy][ipt];
...
//			if (iy!=0) gRFB[iy]->SetPoint(0, exsys[ipt], exsys[ipt], eysys[iy][ipt], eysys[iy][ipt]);
	...
	gRFB[0]->Draw("P");
	//gRFB[1]->SetMarkerSize(2.7);
	gRFB[1]->Draw("P");
	//gRFB[2]->SetMarkerSize(2.7);
	gRFB[2]->Draw("P");
...
//	}
	TLegendEntry *le1=legBR->AddEntry("le1",Form("  %s", rapAbsArr[2].c_str()),"lpf");
	...
	TLegendEntry *le2=legBR->AddEntry("le2",Form("  %s", rapAbsArr[1].c_str()),"lpf");
	...
	TLegendEntry *le3=legBR->AddEntry("le3",Form("  %s", rapAbsArr[0].c_str()),"lpf");
	...
	c1->Update();
	//c1->SaveAs(Form("RFB_%s/RFB_pt_isPrompt%d_%s_bhong.pdf",dirName,(int)isPrompt,runstring.c_str()));
	//c1->SaveAs(Form("RFB_%s/RFB_pt_isPrompt%d_%s_bhong.png",dirName,(int)isPrompt,runstring.c_str()));
	
	///////////////////////////////////////////////////////////////////
	// save as a root file
	TFile *outFile = new TFile(Form("RFB_%s/RFB_pt_isPrompt%d_bhong.root",dirName,(int)isPrompt),"RECREATE");
	outFile->cd();
	for (int iy=0; iy<nRapRFB; iy++){
		gRFB_sys[iy]->Write();	
		gRFB[iy]->Write();	
