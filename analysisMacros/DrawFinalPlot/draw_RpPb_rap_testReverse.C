...
void draw_RpPb_rap_testReverse(bool sysByHand=false, bool noPtWeight=false, bool isPrompt=true)
{
...
//	int iPos=0.;//outOfFrame
  ...
  for (Int_t iy = 0; iy < nRapRpPb; iy++) { 
    ...
    //// testReverse
    DivideValue(pytmp_lowpt_pA[iy+1], eytmp_lowpt_pA[iy+1], pytmp_lowpt_pp[nRapRpPb-iy], eytmp_lowpt_pp[nRapRpPb-iy], &pytmp_lowpt[iy], &eytmp_lowpt[iy]); //actual values
    DivideValue(pytmp_lowpt_pA[iy+1], eysys_lowpt_pA[iy+1], pytmp_lowpt_pp[nRapRpPb-iy], eysys_lowpt_pp[iy], &dummy1, &eysys_lowpt[iy]); // syst.
    ...
    //// testReverse
    DivideValue(pytmp_highpt_pA[iy+1], eytmp_highpt_pA[iy+1], pytmp_highpt_pp[nRapRpPb-iy], eytmp_highpt_pp[nRapRpPb-iy], &pytmp_highpt[iy], &eytmp_highpt[iy]); //actual values
    DivideValue(pytmp_highpt_pA[iy+1], eysys_highpt_pA[iy+1], pytmp_highpt_pp[nRapRpPb-iy], eysys_highpt_pp[nRapRpPb-iy], &dummy1, &eysys_highpt[iy]); // syst.
    //// nomarlize by A_pb = 208.
...
//  g_RpPb_sys_lowpt->GetXaxis()->SetLimits(-2.5,2.1);
  ...
  if (noPtWeight) { 
    c1->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_noPtWeight_pt1.pdf",(int)isPrompt));
    c1->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_noPtWeight_pt1.png",(int)isPrompt));
  } else {
    c1->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_pt1.pdf",(int)isPrompt));
    c1->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_pt1.png",(int)isPrompt));
  }
  ...
  if (noPtWeight) { 
    c2->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_noPtWeight_pt2.pdf",(int)isPrompt));
    c2->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_noPtWeight_pt2.png",(int)isPrompt));
  } else {
    c2->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_pt2.pdf",(int)isPrompt));
    c2->SaveAs(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_pt2.png",(int)isPrompt));
  }
  ...
  //// save as a root file
  TFile *outFile;
  if (noPtWeight) outFile = new TFile(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d_noPtWeight.root",(int)isPrompt),"RECREATE");
  else outFile = new TFile(Form("plot_RpPb_testReverse/RpPb_rap_isPrompt%d.root",(int)isPrompt),"RECREATE");
  outFile->cd();
