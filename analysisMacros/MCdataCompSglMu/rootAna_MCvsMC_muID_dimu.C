...
int rootAna_MCvsMC_muID_dimu(bool isSmall=false)
{
	// # of event range
...
//	TCut totalCutMC = trigCut && recoCut && rapRange && ptRange && accReco;
	...
	TString histTitle[] = {"mass (GeV/c^{2})", "p_{T}^{#mu#mu} (GeV/c)", "y_{lab}", "#phi (rad)"};
	...
	for (int i=0; i<nHist; i++) {
		...
		if (isSmall) {
			tree01->Draw(Form("Reco_QQ_4mom.%s()>>%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
			tree02->Draw(Form("Reco_QQ_4mom.%s()>>%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
		}
		else {
			tree01->Draw(Form("Reco_QQ_4mom.%s()>>%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"");
			tree02->Draw(Form("Reco_QQ_4mom.%s()>>%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"");
		}
		...
		c1[i]->SaveAs(Form("MCvsMC_muID/dblvariables_reco_%s.pdf",histName[i].Data()));
//		c1[i]->SaveAs(Form("SglVariables/sglvariables_%s_%s.png",histName[i].Data(),stringA));
		...
		hRatio[i]->Draw("pe");
		c2[i]->SaveAs(Form("MCvsMC_muID/dblratio_reco_%s.pdf",histName[i].Data()));
	}
/*
	//Save as a root file
