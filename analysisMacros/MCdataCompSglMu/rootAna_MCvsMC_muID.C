...
int rootAna_MCvsMC_muID(bool isSmall=false)
{
	// # of event range
	...
	// read-in root file
	fPrivate = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySamplePrivate/merged_PromptJpsi_PYTHIAboosted_1st_STARTHI53_V27_1Mevt.root");
	fOfficial = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySample/tot_JPsiWithFSR_pa_1st_run_STARTHI53_V27-v1_noAccCut_sglTrig_genMatch-v2.root");
...
//	TString histName[] = {"M", "Pt", "Rapidity","Phi"};
	...
	for (int i=0; i<nHist; i++) {
		...
		if (isSmall) {
			tree01->Draw(Form("Reco_QQ_mupl_4mom.%s()>>%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
			tree01->Draw(Form("Reco_QQ_mumi_4mom.%s()>>+%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
			tree02->Draw(Form("Reco_QQ_mupl_4mom.%s()>>%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
			tree02->Draw(Form("Reco_QQ_mumi_4mom.%s()>>+%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"",nEntry);
		}
		else {
			tree01->Draw(Form("Reco_QQ_mupl_4mom.%s()>>%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"");
			tree01->Draw(Form("Reco_QQ_mumi_4mom.%s()>>+%s",histName[i].Data(),hPrivate[i]->GetName()),totalCutMC && massNarrow,"");
			tree02->Draw(Form("Reco_QQ_mupl_4mom.%s()>>%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"");
			tree02->Draw(Form("Reco_QQ_mumi_4mom.%s()>>+%s",histName[i].Data(),hOfficial[i]->GetName()),totalCutMC && massNarrow,"");
		}
		...
		c1[i]->SaveAs(Form("MCvsMC_muID/sglvariables_reco_%s.pdf",histName[i].Data()));
//		c1[i]->SaveAs(Form("SglVariables/sglvariables_%s_%s.png",histName[i].Data(),stringA));
		...
		hRatio[i]->Draw("pe");
		c2[i]->SaveAs(Form("MCvsMC_muID/sglratio_reco_%s.pdf",histName[i].Data()));
	}
/*
	//Save as a root file
