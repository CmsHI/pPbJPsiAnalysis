...
int rootAna_MCvsMC_muID_gen(bool isSmall=false)
{
	// # of event range
...
//	TString histName[] = {"M", "Pt", "Rapidity","Phi"};
	...
	for (int i=0; i<nHist; i++) {
		...
		c1[i]->SaveAs(Form("MCvsMC_muID/sglvariables_gen_%s.pdf",histName[i].Data()));
//		c1[i]->SaveAs(Form("SglVariables/sglvariables_%s_%s.png",histName[i].Data(),stringA));
		...
		hRatio[i]->Draw("pe");
		c2[i]->SaveAs(Form("MCvsMC_muID/sglratio_gen_%s.pdf",histName[i].Data()));
	}
/*
	//Save as a root file
