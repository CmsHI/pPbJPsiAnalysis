...
int rootAna_acceptance_ptWeight(int MrapNpt=89, int isPA = 1, int accCutType =2, bool isPrompt=true){
  ...
  float ptWeight;
  TFile* inFile = new TFile(Form("dir_fitRatio/%s_fitRatio_%s.root",szPA.Data(),szPrompt.Data()),"READ");
  TF1* funct[nRap];
  for(int iy=0;iy<nRap;iy++){
	  //// iy=0 refers to forwards !!!
    funct[iy]=(TF1*) inFile->Get(Form("funct_%s_%s_%d",szPrompt.Data(),szPA.Data(),iy));
  }

  ////////////////////////////////////////////////////////////////////////////////  
  //// define 2D hist
  //// 1) not weighted
  TH2D *h2D_Den_pt_y_noPtWeight = new TH2D("h2D_Den_pt_y_noPtWeight",";y_{lab};p_{T} (GeV);",nRap,rapArr,nPt,ptArr);
  TH2D *h2D_Num_pt_y_noPtWeight = new TH2D("h2D_Num_pt_y_noPtWeight",";y_{lab};p_{T} (GeV);",nRap,rapArr,nPt,ptArr);
  TH2D *h2D_Acc_pt_y_noPtWeight = new TH2D("h2D_Acc_pt_y_noPtWeight",";y_{lab};p_{T} (GeV);",nRap,rapArr,nPt,ptArr);
  h2D_Den_pt_y_noPtWeight->Sumw2();
  h2D_Num_pt_y_noPtWeight->Sumw2();
  h2D_Acc_pt_y_noPtWeight->Sumw2();
  //// 2) weighted
  ...
  for(int iev=0; iev<nevt; iev++){
    ...
    for (Int_t igqq=0; igqq<Gen_QQ_size; ++igqq) {
      ...
      *dimu_GEN = *mupl_GEN +  *mumi_GEN;
      ...
      //// 1) Denominator
      h2D_Den_pt_y_noPtWeight->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt());
      h2D_Den_pt_y->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt(),ptWeight);
      if (MrapNpt==89) h2D_Den_pt_y_fine->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt(),ptWeight);
      ...
      kineCut(mumi_GEN, accCutType)) {
        h2D_Num_pt_y_noPtWeight->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt());
        h2D_Num_pt_y->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt(),ptWeight);
        if (MrapNpt==89) { h2D_Num_pt_y_fine->Fill(jpsi_GEN->Rapidity(),jpsi_GEN->Pt(),ptWeight); }
      }
    } //end of Gen_QQ_size loop
  }// end of event loop

  //// (Num/Den) to get acceptance (B : binomial error)
  h2D_Acc_pt_y_noPtWeight->Divide(h2D_Num_pt_y_noPtWeight,h2D_Den_pt_y_noPtWeight,1,1,"B");
  ...
  TH2D *h2D_Den_pt_y_noPtWeight_pPb = new TH2D("h2D_Den_pt_y_noPtWeight_pPb","",nRap2nd,rapArr2nd,nPt,ptArr);
  TH2D *h2D_Num_pt_y_noPtWeight_pPb = new TH2D("h2D_Num_pt_y_noPtWeight_pPb","",nRap2nd,rapArr2nd,nPt,ptArr);
  TH2D *h2D_Acc_pt_y_noPtWeight_pPb = new TH2D("h2D_Acc_pt_y_noPtWeight_pPb","",nRap2nd,rapArr2nd,nPt,ptArr);
  h2D_Den_pt_y_noPtWeight_pPb->Sumw2();
  h2D_Num_pt_y_noPtWeight_pPb->Sumw2();
  h2D_Acc_pt_y_noPtWeight_pPb->Sumw2();
  ...
  if (isPA==1) {  
    ...
    ////// same thing for noPtWeight
    for (Int_t iy=0; iy<nRap; iy++){
      for (Int_t ipt=0; ipt<nPt; ipt++) {
        //// Den
        tmpbinDenPbp = h2D_Den_pt_y_noPtWeight->FindBin((rapArr[iy]+rapArr[iy+1])/2, (ptArr[ipt]+ptArr[ipt+1])/2);
        tmpDenVal = h2D_Den_pt_y_noPtWeight->GetBinContent(tmpbinDenPbp);
        tmpDenErr = h2D_Den_pt_y_noPtWeight->GetBinError(tmpbinDenPbp);
        tmpbinDenpPb = h2D_Den_pt_y_noPtWeight_pPb->FindBin((rapArr2nd[nRap-iy-1]+rapArr2nd[nRap-iy])/2,(ptArr[ipt]+ptArr[ipt+1])/2);
        h2D_Den_pt_y_noPtWeight_pPb->SetBinContent(tmpbinDenpPb,tmpDenVal);
        h2D_Den_pt_y_noPtWeight_pPb->SetBinError(tmpbinDenpPb,tmpDenErr);
        //// Num  
        tmpbinNumPbp = h2D_Num_pt_y_noPtWeight->FindBin((rapArr[iy]+rapArr[iy+1])/2, (ptArr[ipt]+ptArr[ipt+1])/2);
        tmpNumVal = h2D_Num_pt_y_noPtWeight->GetBinContent(tmpbinNumPbp);
        tmpNumErr = h2D_Num_pt_y_noPtWeight->GetBinError(tmpbinNumPbp);
        tmpbinNumpPb = h2D_Num_pt_y_noPtWeight_pPb->FindBin((rapArr2nd[nRap-iy-1]+rapArr2nd[nRap-iy])/2,(ptArr[ipt]+ptArr[ipt+1])/2);
        h2D_Num_pt_y_noPtWeight_pPb->SetBinContent(tmpbinNumpPb,tmpNumVal);
        h2D_Num_pt_y_noPtWeight_pPb->SetBinError(tmpbinNumpPb,tmpNumErr);
        //// Acc  
        tmpbinAccPbp = h2D_Acc_pt_y_noPtWeight->FindBin((rapArr[iy]+rapArr[iy+1])/2, (ptArr[ipt]+ptArr[ipt+1])/2);
        tmpAccVal = h2D_Acc_pt_y_noPtWeight->GetBinContent(tmpbinAccPbp);
        tmpAccErr = h2D_Acc_pt_y_noPtWeight->GetBinError(tmpbinAccPbp);
        tmpbinAccpPb = h2D_Acc_pt_y_noPtWeight_pPb->FindBin((rapArr2nd[nRap-iy-1]+rapArr2nd[nRap-iy])/2,(ptArr[ipt]+ptArr[ipt+1])/2);
        h2D_Acc_pt_y_noPtWeight_pPb->SetBinContent(tmpbinAccpPb,tmpAccVal);
        h2D_Acc_pt_y_noPtWeight_pPb->SetBinError(tmpbinAccpPb,tmpAccErr);
      }
    }
  }
  ////////////////////////////////////////////////////////////////////////////////////////////////////  
  ...
  if (isPA==1) {
    h2D_Den_pt_y_noPtWeight->SetName("h2D_Den_pt_y_noPtWeight_Pbp");
    h2D_Num_pt_y_noPtWeight->SetName("h2D_Num_pt_y_noPtWeight_Pbp");
    h2D_Acc_pt_y_noPtWeight->SetName("h2D_Acc_pt_y_noPtWeight_Pbp");
    h2D_Den_pt_y->SetName("h2D_Den_pt_y_Pbp");
    h2D_Num_pt_y->SetName("h2D_Num_pt_y_Pbp");
    h2D_Acc_pt_y->SetName("h2D_Acc_pt_y_Pbp");
  }

  TFile *outFile = new TFile(Form("AccAna_%s_ptWeight.root",szFinal.Data()),"RECREATE");
  outFile->cd();
  h2D_Den_pt_y_noPtWeight->Write();
  h2D_Num_pt_y_noPtWeight->Write();
  h2D_Acc_pt_y_noPtWeight->Write();
  h2D_Den_pt_y->Write();
  h2D_Num_pt_y->Write();
  h2D_Acc_pt_y->Write();
  if (isPA==1) {
    h2D_Den_pt_y_noPtWeight_pPb->Write();
    h2D_Num_pt_y_noPtWeight_pPb->Write();
    h2D_Acc_pt_y_noPtWeight_pPb->Write();
