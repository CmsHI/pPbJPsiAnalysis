...
float getRecoWeight(float dimu_pt, float dimu_y, bool isPr, bool isPbp);
...
//TFile* fRatio_PRPbp = new TFile("fitRatio_PRPbp.root");
TFile* fRatio_PRPbp = new TFile("fitRatio_PRPbp_isStiff1.root");
TF1* f_ptRatio_PRPbp_0=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_0");
TF1* f_ptRatio_PRPbp_1=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_1");
TF1* f_ptRatio_PRPbp_2=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_2");
TF1* f_ptRatio_PRPbp_3=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_3");
TF1* f_ptRatio_PRPbp_4=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_4");
TF1* f_ptRatio_PRPbp_5=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_5");
TF1* f_ptRatio_PRPbp_6=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_6");
TF1* f_ptRatio_PRPbp_7=(TF1*)fRatio_PRPbp->Get("f_ptRatio_PRPbp_7");

/*
TFile* fRatio_NPPbp = new TFile("fitRatio_NPPbp.root");
TF1* f_ptRatio_NPPbp_0=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_0");
TF1* f_ptRatio_NPPbp_1=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_1");
TF1* f_ptRatio_NPPbp_2=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_2");
TF1* f_ptRatio_NPPbp_3=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_3");
TF1* f_ptRatio_NPPbp_4=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_4");
TF1* f_ptRatio_NPPbp_5=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_5");
TF1* f_ptRatio_NPPbp_6=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_6");
TF1* f_ptRatio_NPPbp_7=(TF1*)fRatio_NPPbp->Get("f_ptRatio_NPPbp_7");
*/

/////// main func. ///////
int rootAna_efficiency_8rap9pt_ptWeight_isStiff1(char *strBinning = "8rap9pt", bool isPrompt = true, bool is1st = true, bool useCtErrCut =true, bool useSF=true, bool useZvtxWeight=true){
  ...
  mytree->SetBranchAddress("Gen_QQ_mumi_4mom", &Gen_QQ_mumi_4mom, &b_Gen_QQ_mumi_4mom);
	...
	TH2D *h2D_Den_noPtWeight_pt_y = new TH2D("h2D_Den_noPtWeight_pt_y","",nRapBins,rapBinsArr,nPtBins,ptBinsArr);
	TH2D *h2D_Num_noPtWeight_pt_y = new TH2D("h2D_Num_noPtWeight_pt_y","",nRapBins,rapBinsArr,nPtBins,ptBinsArr);
	TH2D *h2D_Eff_noPtWeight_pt_y = new TH2D("h2D_Eff_noPtWeight_pt_y","",nRapBins,rapBinsArr,nPtBins,ptBinsArr);
	TH2D* h2D_Eff_diff_pt_y= new TH2D("h2D_Eff_diff_pt_y","",nRapBins,rapBinsArr,nPtBins,ptBinsArr);	
	TH2D* h2D_Eff_err_pt_y= new TH2D("h2D_Eff_err_pt_y","",nRapBins,rapBinsArr,nPtBins,ptBinsArr);	
	...
	h2D_Num_pt_y->Sumw2();
	h2D_Eff_pt_y->Sumw2();
	h2D_Den_noPtWeight_pt_y->Sumw2();
	h2D_Num_noPtWeight_pt_y->Sumw2();
	h2D_Eff_noPtWeight_pt_y->Sumw2();
	    ...
	    *JP_Gen_tmp = *m1P_Gen +  *m2P_Gen; // Gen dimuon pairs (for filling NoCut)
	    ...
	    if ( kineCut(Jpsi_Gen.mupl_pt, Jpsi_Gen.mupl_eta, Jpsi_Gen.mupl_p) 
	      ...
	      *JP_Gen = *m1P_Gen +  *m2P_Gen; // used for actual denominator ( GEN dimuon pairs)
			...
			recoWeight =1.;
	    if (yn_gen == true){
				recoWeight = getRecoWeight(Jpsi_Gen.thePt, Jpsi_Gen.theRapidity, isPrompt, is1st);
				h2D_Den_pt_y->Fill(Jpsi_Gen.theRapidity,Jpsi_Gen.thePt,zWeight*recoWeight);
				h2D_Den_noPtWeight_pt_y->Fill(Jpsi_Gen.theRapidity,Jpsi_Gen.thePt,zWeight);
	    } // end of yn_gen
	    ...
	    && minylab<=Jpsi_Reco.theRapidity && Jpsi_Reco.theRapidity < maxylab) {
	  	...
	  	recoWeight = 1;
	    	...
	    	if (useSF){
				...
				recoWeight = getRecoWeight(Jpsi_Reco.thePt, Jpsi_Reco.theRapidity, isPrompt, is1st);
				h2D_Num_pt_y->Fill(Jpsi_Reco.theRapidity,Jpsi_Reco.thePt, tnpWeight*zWeight*recoWeight);
				h2D_Num_noPtWeight_pt_y->Fill(Jpsi_Reco.theRapidity,Jpsi_Reco.thePt, tnpWeight*zWeight);
				h1D_zVtx->Fill(theZvtx, zWeight);
			}
	...
	h2D_Eff_noPtWeight_pt_y->Divide(h2D_Num_noPtWeight_pt_y,h2D_Den_noPtWeight_pt_y,1,1,"B");

	////////////////////////////////////////////////////////////////////////////
	// difference and relative error calculation
	for(int ipt=0;ipt<nPtBins;ipt++){
		for(int iy=0;iy<nRapBins;iy++){
			h2D_Eff_diff_pt_y = (TH2D*) h2D_Eff_noPtWeight_pt_y->Clone("h2D_Eff_diff_pt_y");
	...
	// --- save as a root file
	TFile *outFile = new TFile(Form("EffAna_%s_CtErr%d_SF%d_Zvtx%d_ptWeight_isStiff1.root",strName, (int)useCtErrCut ,(int)useSF, (int)useZvtxWeight),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
	h2D_ctErrmin->Write();
	h2D_ctErrmax->Write();
	h2D_Den_noPtWeight_pt_y->Write();
	h2D_Num_noPtWeight_pt_y->Write();
	h2D_Eff_noPtWeight_pt_y->Write();
	h2D_Den_pt_y->Write();
	h2D_Num_pt_y->Write();
	h2D_Eff_pt_y->Write();
...
float getRecoWeight(float dimu_pt, float dimu_y, bool isPr, bool isPbp) { 
	...
	TF1* t_func;
		if(isPbp){
			if(-2.40<= dimu_y && dimu_y<-1.97) {  t_func=f_ptRatio_PRPbp_0; ptThr=2.0; }
			else if(-1.97<= dimu_y && dimu_y<-1.37) {	t_func=f_ptRatio_PRPbp_1;	ptThr=3.0; }
			else if(-1.37<= dimu_y && dimu_y<-0.47) {	t_func=f_ptRatio_PRPbp_2;	ptThr=6.5; }
			else if(-0.47<= dimu_y && dimu_y<0.43) {	t_func=f_ptRatio_PRPbp_3;	ptThr=6.5; }
			else if(0.43<= dimu_y && dimu_y<1.03) {	t_func=f_ptRatio_PRPbp_4;	ptThr=6.5; }
			else if(1.03<= dimu_y && dimu_y<1.46) {	t_func=f_ptRatio_PRPbp_5;	ptThr=5.0; }
			else if(1.46<= dimu_y && dimu_y<1.93) {	t_func=f_ptRatio_PRPbp_6;	ptThr=3.0; }
			else if(1.93<= dimu_y && dimu_y<2.40) {	t_func=f_ptRatio_PRPbp_7;	ptThr=2.0; }
		} else {
			if(-2.40<= dimu_y && dimu_y<-1.93) {	t_func=f_ptRatio_PRPbp_7; ptThr=2.0; }
			else if(-1.93<= dimu_y && dimu_y<-1.46) {	t_func=f_ptRatio_PRPbp_6;	ptThr=3.0; }
			else if(-1.46<= dimu_y && dimu_y<-1.03) {	t_func=f_ptRatio_PRPbp_5;	ptThr=5.0; }
			else if(-1.03<= dimu_y && dimu_y<-0.43) {	t_func=f_ptRatio_PRPbp_4;	ptThr=6.5; }
			else if(-0.43<= dimu_y && dimu_y<0.47) {	t_func=f_ptRatio_PRPbp_3;	ptThr=6.5; }
			else if(0.47<= dimu_y && dimu_y<1.37) {	t_func=f_ptRatio_PRPbp_2;	ptThr=6.5; }
			else if(1.37<= dimu_y && dimu_y<1.97) {	t_func=f_ptRatio_PRPbp_1;	ptThr=3.0; }
			else if(1.97<= dimu_y && dimu_y<2.40) {	t_func=f_ptRatio_PRPbp_0;	ptThr=2.0; }
		}	
	// always use prompt Pbp (also for non-prompt)
	/*
	else {
		if(isPbp){
			if(-2.40<= dimu_y && dimu_y<-1.97) {  t_func=f_ptRatio_NPPbp_0; ptThr=2.0; }
			else if(-1.97<= dimu_y && dimu_y<-1.37) {	t_func=f_ptRatio_NPPbp_1;	ptThr=3.0; }
			else if(-1.37<= dimu_y && dimu_y<-0.47) {	t_func=f_ptRatio_NPPbp_2;	ptThr=6.5; }
			else if(-0.47<= dimu_y && dimu_y<0.43) {	t_func=f_ptRatio_NPPbp_3;	ptThr=6.5; }
			else if(0.43<= dimu_y && dimu_y<1.03) {	t_func=f_ptRatio_NPPbp_4;	ptThr=6.5; }
			else if(1.03<= dimu_y && dimu_y<1.46) {	t_func=f_ptRatio_NPPbp_5;	ptThr=5.0; }
			else if(1.46<= dimu_y && dimu_y<1.93) {	t_func=f_ptRatio_NPPbp_6;	ptThr=3.0; }
			else if(1.93<= dimu_y && dimu_y<2.40) {	t_func=f_ptRatio_NPPbp_7;	ptThr=2.0; }
		} else {
			if(-2.40<= dimu_y && dimu_y<-1.93) {	t_func=f_ptRatio_NPPbp_7; ptThr=2.0; }
			else if(-1.93<= dimu_y && dimu_y<-1.46) {	t_func=f_ptRatio_NPPbp_6;	ptThr=3.0; }
			else if(-1.46<= dimu_y && dimu_y<-1.03) {	t_func=f_ptRatio_NPPbp_5;	ptThr=5.0; }
			else if(-1.03<= dimu_y && dimu_y<-0.43) {	t_func=f_ptRatio_NPPbp_4;	ptThr=6.5; }
			else if(-0.43<= dimu_y && dimu_y<0.47) {	t_func=f_ptRatio_NPPbp_3;	ptThr=6.5; }
			else if(0.47<= dimu_y && dimu_y<1.37) {	t_func=f_ptRatio_NPPbp_2;	ptThr=6.5; }
			else if(1.37<= dimu_y && dimu_y<1.97) {	t_func=f_ptRatio_NPPbp_1;	ptThr=3.0; }
			else if(1.97<= dimu_y && dimu_y<2.40) {	t_func=f_ptRatio_NPPbp_0;	ptThr=2.0; }
		}	
	}
	*/
	float recWeight = t_func->Eval(dimu_pt);
	return recWeight;
}
