...
int cndr_rootAna_meanPt_wBkg_5(TString szPA="pa", bool isSmall=true){
  ...
  //TCut totalCutMC = recoPairCut && rapRange && ptRange && accNewRecoPlus && accNewRecoMinus;
	...
	for (int in=0; in<nEt; in++){
        ...
        if (isSmall) {
			...
			cmulti[in][ir]->Update();
			cmulti[in][ir]->SaveAs(Form("dir_meanPt_n%d_%dpt/meanPt_%dpt_%d_%d_wBkg_5.pdf",nbin,nPt,nPt,in,ir));
			//cmulti[in][ir]->Clear();
		}
	}
	
	/// Save as a root file
	TFile *outFile = new TFile(Form("dir_meanPt_n%d_%dpt/meanPt_%dpt_wBkg_5.root",nbin,nPt,nPt),"RECREATE");
	outFile->cd();
	for (int in=0; in<nEt; in++){
		for (int ir=0; ir<nRap; ir++ ) {
			for (int ipt=0; ipt<nPt; ipt++) {
