...
int comp_crossSection_pt_LHCb_middle_xErr(bool shifted=false, Int_t runCode=0, bool isScale =false, bool isLog=true, bool isPtCut = false)
{
...
//	}
	...
	c_pr->Update();
	c_pr->SaveAs(Form("comparisonLHCb/comp_crossSection_pt_LHCb_pr_isLog%d_isPtCut_%d_shifted_%d_middle_xErr.pdf",(int)isLog,(int)isPtCut,(int)shifted));
	c_pr->SaveAs(Form("comparisonLHCb/comp_crossSection_pt_LHCb_pr_isLog%d_isPtCut_%d_shifted_%d_middle_xErr.png",(int)isLog,(int)isPtCut,(int)shifted));
	legUR->Clear();
	//c_pr->Clear();
...
//		latex->DrawLatex(0.89, 0.31, "Preliminary");
	...
	c_np->Update();
	c_np->SaveAs(Form("comparisonLHCb/comp_crossSection_pt_LHCb_np_isLog%d_isPtCut_%d_shifted_%d_middle_xErr.pdf",(int)isLog,(int)isPtCut,(int)shifted));
	c_np->SaveAs(Form("comparisonLHCb/comp_crossSection_pt_LHCb_np_isLog%d_isPtCut_%d_shifted_%d_middle_xErr.png",(int)isLog,(int)isPtCut,(int)shifted));
	//legUR->Clear();
	//c_np->Clear();
