...
void read_Fit_pt_forExcel(TString szBinning="8rap9pt", int etOpt=1, int isPA=1, bool isPrompt=true)
{
	gROOT->Macro("../Style.C");
  ...
  f2D_02 = new TFile(Form("../TotalSys/TotSys_%s_%s_etOpt%d.root",szBinning.Data(),szPA.Data(),etOpt));
	...
	TH2D* h2D_01 = (TH2D*)f2D_01->Get(Form("h2D_Fit_%s_%s",szPrompt.Data(),szPA.Data()));
	TH2D* h2D_02 = (TH2D*)f2D_02->Get(Form("hFit%s",szPrompt.Data()));
