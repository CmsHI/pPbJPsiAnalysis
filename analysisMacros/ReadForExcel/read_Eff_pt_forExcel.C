...
void read_Eff_pt_forExcel(TString szBinning="8rap9pt", int etOpt=0, int isPA=1, bool isPrompt=true)
{
	gROOT->Macro("../Style.C");
  ...
  f2D_02 = new TFile(Form("../TotalSys/TotSys_%s_%s_etOpt%d.root",szBinning.Data(),szPA.Data(),etOpt));
	...
	TH2D* h2D_02_A = (TH2D*)f2D_02->Get(Form("hEffMC%s",szPrompt.Data()));
	TH2D* h2D_02_B = (TH2D*)f2D_02->Get(Form("hTNP%s",szPrompt.Data()));
	TH2D* h2D_02 = (TH2D*)f2D_02->Get(Form("hEff%s",szPrompt.Data()));
	cout << "h2D_01 : " << h2D_01->GetName() << endl;
	cout << "h2D_02_A : " << h2D_02_A->GetName() << endl;
	cout << "h2D_02_B : " << h2D_02_B->GetName() << endl;
    ...
    if (isPA==0){
      ...
      h1D_02_A[iy] = h2D_02_A->ProjectionY(Form("h1D_02_A_%d",iy),nRap-iy,nRap-iy);
      h1D_02_B[iy] = h2D_02_B->ProjectionY(Form("h1D_02_B_%d",iy),nRap-iy,nRap-iy);
      h1D_02[iy] = h2D_02->ProjectionY(Form("h1D_02_%d",iy),nRap-iy,nRap-iy);
		}
    else {
      h1D_01[iy] = h2D_01->ProjectionY(Form("h1D_01_%d",iy),iy+1,iy+1);
      h1D_02_A[iy] = h2D_02_A->ProjectionY(Form("h1D_02_A_%d",iy),iy+1,iy+1);
      h1D_02_B[iy] = h2D_02_B->ProjectionY(Form("h1D_02_B_%d",iy),iy+1,iy+1);
  ...
  cout << "::: value // stat // rel MC // rel TNP // rel syst " << endl;
  for (Int_t iy = 0; iy < nRap; iy++) {
    cout << "::: for excel ::: iy= " << iy << endl;
		for (Int_t ipt=0; ipt<nPt; ipt++ ){
      cout << h1D_01[iy]->GetBinContent(ipt+1) <<"\t"<<h1D_01[iy]->GetBinError(ipt+1) << "\t"<<
      TMath::Abs(h1D_02_A[iy]->GetBinContent(ipt+1))<< "\t"<<
      TMath::Abs(h1D_02_B[iy]->GetBinContent(ipt+1))<< "\t"<<
      TMath::Abs(h1D_02[iy]->GetBinContent(ipt+1))<<endl;
    }
  }
