...
void draw_1D_eff_Zvtx1_Zvtx0(char* szBinning="8rap9pt", bool isLog=false)
{
	...
	TH2D* h2D_Pbp = (TH2D*)f2D_PA->Get("h2D_Eff_PR_Pbp");
	TH2D* h2D_pPb = (TH2D*)f2D_PA->Get("h2D_Eff_PR_pPb");
	TH2D* h2D_pp = (TH2D*)f2D_PP->Get("h2D_Eff_PR_pp");
	cout << "h2D_pA = " << h2D_pA << endl;
	cout << "h2D_Pbp = " << h2D_Pbp << endl;
	cout << "h2D_pPb = " << h2D_pPb << endl;
	cout << "h2D_pp = " << h2D_pp << endl;

	const int nbinsX = h2D_Pbp->GetNbinsX();
	const int nbinsY = h2D_Pbp->GetNbinsY();
...
//	}
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		//SetHistStyle(h1D_Pbp[iy],1,10);
		//SetHistStyle(h1D_pPb[iy],2,10);
		SetHistStyle(h1D_Pbp[iy],3,10);
		SetHistStyle(h1D_pPb[iy],5,10);
		//SetHistStyle(h1D_pp[iy],6,3);
		SetHistStyle(h1D_pp[iy],6,13);
		
		h1D_Pbp[iy]->GetXaxis()->SetTitle("p_{T} (GeV/c)");
		h1D_Pbp[iy]->GetXaxis()->CenterTitle();
		h1D_Pbp[iy]->GetXaxis()->SetRangeUser(0.,30.);
		h1D_Pbp[iy]->GetYaxis()->SetTitle("Efficiency");
		h1D_Pbp[iy]->GetYaxis()->SetRangeUser(0,1.0);
		//h1D_Pbp[iy]->GetYaxis()->SetRangeUser(0,0.6);
