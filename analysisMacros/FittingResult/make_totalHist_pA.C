...
int make_totalHist_pA(int MrapNpt=89, int accCutType = 2, bool useZvtxWeight=true, bool useSF = true, bool noPtWeight = true, TString szSys="nominal", int etOpt=0){
  ...
  TFile *fAccPR = new TFile(Form("../AccEffWeight/AccAna_%s_pA_PR_%s_ptWeight.root",szBinning.Data(),szAccCut.Data()));
  TFile *fAccNP = new TFile(Form("../AccEffWeight/AccAna_%s_pA_NP_%s_ptWeight.root",szBinning.Data(),szAccCut.Data()));
  //////////////////////// Acc :: directly get Pbp as pA
  TH2D* h2D_Acc_Den_PR_pA; TH2D* h2D_Acc_Num_PR_pA; TH2D* h2D_Acc_PR_pA;
  TH2D* h2D_Acc_Den_NP_pA; TH2D* h2D_Acc_Num_NP_pA; TH2D* h2D_Acc_NP_pA;
  if (noPtWeight) {
    h2D_Acc_Den_PR_pA = (TH2D*)fAccPR->Get("h2D_Den_pt_y_noPtWeight_Pbp");
    h2D_Acc_Den_NP_pA = (TH2D*)fAccNP->Get("h2D_Den_pt_y_noPtWeight_Pbp");
    h2D_Acc_Num_PR_pA = (TH2D*)fAccPR->Get("h2D_Num_pt_y_noPtWeight_Pbp");
    h2D_Acc_Num_NP_pA = (TH2D*)fAccNP->Get("h2D_Num_pt_y_noPtWeight_Pbp");
    h2D_Acc_PR_pA = (TH2D*)fAccPR->Get("h2D_Acc_pt_y_noPtWeight_Pbp");
    h2D_Acc_NP_pA = (TH2D*)fAccNP->Get("h2D_Acc_pt_y_noPtWeight_Pbp");
  } else {
    h2D_Acc_Den_PR_pA = (TH2D*)fAccPR->Get("h2D_Den_pt_y_Pbp");
    h2D_Acc_Den_NP_pA = (TH2D*)fAccNP->Get("h2D_Den_pt_y_Pbp");
    h2D_Acc_Num_PR_pA = (TH2D*)fAccPR->Get("h2D_Num_pt_y_Pbp");
    h2D_Acc_Num_NP_pA = (TH2D*)fAccNP->Get("h2D_Num_pt_y_Pbp");
    h2D_Acc_PR_pA = (TH2D*)fAccPR->Get("h2D_Acc_pt_y_Pbp");
    h2D_Acc_NP_pA = (TH2D*)fAccNP->Get("h2D_Acc_pt_y_Pbp");
  }
  ...
  TFile *fEffPRPbp = new TFile(Form("../AccEffWeight/EffAna_%s_Pbp_PR_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  TFile *fEffPRpPb = new TFile(Form("../AccEffWeight/EffAna_%s_pPb_PR_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  TFile *fEffNPPbp = new TFile(Form("../AccEffWeight/EffAna_%s_Pbp_NP_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  TFile *fEffNPpPb = new TFile(Form("../AccEffWeight/EffAna_%s_pPb_NP_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  
  TH2D* h2D_Eff_Den_PR_Pbp; TH2D* h2D_Eff_Num_PR_Pbp; TH2D* h2D_Eff_PR_Pbp;
  TH2D* h2D_Eff_Den_NP_Pbp; TH2D* h2D_Eff_Num_NP_Pbp; TH2D* h2D_Eff_NP_Pbp;
  TH2D* h2D_Eff_Den_PR_pPb; TH2D* h2D_Eff_Num_PR_pPb; TH2D* h2D_Eff_PR_pPb;
  TH2D* h2D_Eff_Den_NP_pPb; TH2D* h2D_Eff_Num_NP_pPb; TH2D* h2D_Eff_NP_pPb;
  
  if (noPtWeight) {
    h2D_Eff_Den_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Den_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Num_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_Num_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Eff_pt_y_noPtWeight");
    h2D_Eff_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Eff_pt_y_noPtWeight");
    h2D_Eff_Den_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Den_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Num_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_Num_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Eff_pt_y_noPtWeight");
    h2D_Eff_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Eff_pt_y_noPtWeight");
  } else {
    h2D_Eff_Den_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Den_pt_y");
    h2D_Eff_Den_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Den_pt_y");
    h2D_Eff_Num_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Num_pt_y");
    h2D_Eff_Num_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Num_pt_y");
    h2D_Eff_PR_Pbp = (TH2D*)fEffPRPbp->Get("h2D_Eff_pt_y");
    h2D_Eff_NP_Pbp = (TH2D*)fEffNPPbp->Get("h2D_Eff_pt_y");
    h2D_Eff_Den_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Den_pt_y");
    h2D_Eff_Den_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Den_pt_y");
    h2D_Eff_Num_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Num_pt_y");
    h2D_Eff_Num_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Num_pt_y");
    h2D_Eff_PR_pPb = (TH2D*)fEffPRpPb->Get("h2D_Eff_pt_y");
    h2D_Eff_NP_pPb = (TH2D*)fEffNPpPb->Get("h2D_Eff_pt_y");
  }
  ...
  TFile* fFitpA = new TFile(Form("./fitResHist_%s_pA_%s_%s_etOpt%d.root",szBinning.Data(),szAccCut.Data(),szSys.Data(),etOpt));
  ...
  TFile *outFile;
  if (noPtWeight) {
    outFile = new TFile(Form("totalHist_pA_%s_Zvtx%d_SF%d_etOpt%d_noPtWeight.root",szFinal.Data(),(int)useZvtxWeight,(int)useSF,etOpt),"RECREATE");
    cout << "totalHist_pA_"<<szFinal<<"_Zvtx"<<(int)useZvtxWeight<<"_SF"<<(int)useSF<<"_etOpt"<<etOpt<<"_noPtWeight.root has been created :) " <<endl;
  } else {
    outFile = new TFile(Form("totalHist_pA_%s_Zvtx%d_SF%d_etOpt%d.root",szFinal.Data(),(int)useZvtxWeight,(int)useSF,etOpt),"RECREATE");
    cout << "totalHist_pA_"<<szFinal<<"_Zvtx"<<(int)useZvtxWeight<<"_SF"<<(int)useSF<<"_etOpt"<<etOpt<<".root has been created :) " <<endl;
  }
