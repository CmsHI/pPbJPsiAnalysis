...
void draw_1D_yield(TString szBinning="8rap9pt", int isPA=1, bool isPrompt=true, bool isLog=false, TString szParam="coefExp")
{
	gROOT->Macro("../Style.C");
...
//		}
	...
	c01->Modified();
	c01->Update();
	c01->SaveAs(Form("dir_1D_yield/%s_%s_%s_wide.pdf",szPA.Data(),szBinning.Data(),szParam.Data()));
  ...
  for (Int_t iy = 0; iy < nbinsX; iy++) {
	...
	legUR->Draw();
	c02->SaveAs(Form("dir_1D_yield/%s_%s_%s.pdf",szPA.Data(),szBinning.Data(),szParam.Data()));
