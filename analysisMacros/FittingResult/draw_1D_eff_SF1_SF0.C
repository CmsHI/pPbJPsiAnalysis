...
void draw_1D_eff_SF1_SF0(TString szBinning="8rap9pt", int isPA=0, bool isPrompt=false, bool isLog=false, bool isNoErr=true)
{
  ...
  if (isPA==0) {
	...
	TH2D* h2D_02 = (TH2D*)f2D_02->Get(Form("h2D_Eff_%s_%s",szPrompt.Data(),szPA.Data()));
    ...
    if (isPA==0){
      h1D_01[iy] = h2D_01->ProjectionY(Form("h1D_01_%d",iy),nbinsX-iy,nbinsX-iy);
  		h1D_02[iy] = h2D_02->ProjectionY(Form("h1D_02_%d",iy),nbinsX-iy,nbinsX-iy);
		}
    else {
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		if (iy>=1 && iy<=6) {
			...
			h1D_02[iy]->SetBinContent(1,+532);
			h1D_02[iy]->SetBinError(1,0);
			h1D_01[iy]->SetBinContent(2,-532);
			h1D_01[iy]->SetBinError(2,0);
			h1D_02[iy]->SetBinContent(2,+532);
			h1D_02[iy]->SetBinError(2,0);
		}
		if (iy>=2 && iy<=5) {
			h1D_01[iy]->SetBinContent(3,-532);
			h1D_01[iy]->SetBinError(3,0);
			h1D_02[iy]->SetBinContent(3,+532);
			h1D_02[iy]->SetBinError(3,0);
		}
    if (isPA ==0){ //for_pp 
      if (iy>=2 && iy<=5) { 
			  h1D_01[iy]->SetBinContent(4,-532);
			  h1D_01[iy]->SetBinError(4,0);
			  h1D_02[iy]->SetBinContent(4,+532);
			  h1D_02[iy]->SetBinError(4,0);
		  }
    }
    else {
      if (iy>=2 && iy<=4) { // for_pA
...
/*	
  ////// after zero-bin setting, normalize!
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		h1D_02[iy]->Scale(1./h1D_02[iy]->Integral());
		h1D_02[iy]->Scale(1,"width");
	}
*/
  ...
  TH1D* hRatio[nRap];
    ...
    if (isNoErr) {
      for (int ipt=0; ipt<nbinsY; ipt++) {
        ...
        cout << iy <<"th rap, "<<ipt<<"th pT, ratio = "<<hRatio[iy]->GetBinContent(ipt+1)<<endl;
      }
    }
  }
  ...
  TGraphAsymmErrors* g_02[nbinsX];
	...
	for (Int_t iy = 0; iy < nbinsX; iy++) {
		...
		g_02[iy]=new TGraphAsymmErrors(h1D_02[iy]);
    ...
    c01->cd(iy+1);
		...
		g_01[iy]->GetYaxis()->SetRangeUser(0,1.0);
		
		g_01[iy]->Draw("AP");
		g_02[iy]->Draw("P");
		if (iy==0) {
			legBR -> AddEntry(g_01[iy],"SF","lp");
			legBR -> AddEntry(g_02[iy],"no SF","lp");
			legBR->Draw();
			...
			if (isPrompt) { latex->DrawLatex(0.55,0.85,Form("%s Prompt J/#psi",szPA.Data())); }
			else { latex->DrawLatex(0.55,0.85,Form("%s Non-prompt J/#psi",szPA.Data())); }
		}
...
*/
		c02->cd(iy+1);
		gRatio[iy]->GetXaxis()->SetLimits(0.0,30.0);
		...
		gRatio[iy]->GetYaxis()->SetTitle("[Weight] / [No weight]");
		gRatio[iy]->Draw("AP");
		if (iy==0){
			if (isPrompt) { latex->DrawLatex(0.19,0.23,Form("%s Prompt J/#psi",szPA.Data())); }
			else { latex->DrawLatex(0.19,0.23,Form("%s Non-prompt J/#psi",szPA.Data())); }
