...
/////// main func. ///////
int make_totalHist_pp(int MrapNpt=89, int accCutType = 2, bool useZvtxWeight=false, bool useSF = true, bool noPtWeight = true, TString szSys="nominal", int etOpt=0){
  using namespace std;
  ...
  TFile *fAccPR = new TFile(Form("../AccEffWeight/AccAna_%s_pp_PR_%s_ptWeight.root",szBinning.Data(),szAccCut.Data()));
  TFile *fAccNP = new TFile(Form("../AccEffWeight/AccAna_%s_pp_NP_%s_ptWeight.root",szBinning.Data(),szAccCut.Data()));
  TH2D* h2D_Acc_Den_PR_pp; TH2D* h2D_Acc_Num_PR_pp; TH2D* h2D_Acc_PR_pp;
  TH2D* h2D_Acc_Den_NP_pp; TH2D* h2D_Acc_Num_NP_pp; TH2D* h2D_Acc_NP_pp;
  if (noPtWeight) {
    h2D_Acc_Den_PR_pp = (TH2D*)fAccPR->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Acc_Den_NP_pp = (TH2D*)fAccNP->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Acc_Num_PR_pp = (TH2D*)fAccPR->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Acc_Num_NP_pp = (TH2D*)fAccNP->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Acc_PR_pp = (TH2D*)fAccPR->Get("h2D_Acc_pt_y_noPtWeight");
    h2D_Acc_NP_pp = (TH2D*)fAccNP->Get("h2D_Acc_pt_y_noPtWeight");
  } else {
    h2D_Acc_Den_PR_pp = (TH2D*)fAccPR->Get("h2D_Den_pt_y");
    h2D_Acc_Den_NP_pp = (TH2D*)fAccNP->Get("h2D_Den_pt_y");
    h2D_Acc_Num_PR_pp = (TH2D*)fAccPR->Get("h2D_Num_pt_y");
    h2D_Acc_Num_NP_pp = (TH2D*)fAccNP->Get("h2D_Num_pt_y");
    h2D_Acc_PR_pp = (TH2D*)fAccPR->Get("h2D_Acc_pt_y");
    h2D_Acc_NP_pp = (TH2D*)fAccNP->Get("h2D_Acc_pt_y");
  }
  ...
  TFile *fEffPRpp = new TFile(Form("../AccEffWeight/EffAna_%s_pp_PR_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  TFile *fEffNPpp = new TFile(Form("../AccEffWeight/EffAna_%s_pp_NP_%s_Zvtx%d_SF%d_ptWeight.root",szBinning.Data(),szAccCut.Data(),(int)useZvtxWeight,(int)useSF));
  TH2D* h2D_Eff_Den_PR_pp; TH2D* h2D_Eff_Num_PR_pp; TH2D* h2D_Eff_PR_pp;
  TH2D* h2D_Eff_Den_NP_pp; TH2D* h2D_Eff_Num_NP_pp; TH2D* h2D_Eff_NP_pp;
  if (noPtWeight) {
    h2D_Eff_Den_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Den_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Den_pt_y_noPtWeight");
    h2D_Eff_Num_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_Num_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Num_pt_y_noPtWeight");
    h2D_Eff_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Eff_pt_y_noPtWeight");
    h2D_Eff_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Eff_pt_y_noPtWeight");
  } else {
    h2D_Eff_Den_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Den_pt_y");
    h2D_Eff_Den_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Den_pt_y");
    h2D_Eff_Num_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Num_pt_y");
    h2D_Eff_Num_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Num_pt_y");
    h2D_Eff_PR_pp = (TH2D*)fEffPRpp->Get("h2D_Eff_pt_y");
    h2D_Eff_NP_pp = (TH2D*)fEffNPpp->Get("h2D_Eff_pt_y");
  }
  ...
  TFile* fFitpp = new TFile(Form("./fitResHist_%s_pp_%s_%s_etOpt%d.root",szBinning.Data(),szAccCut.Data(),szSys.Data(),etOpt));
  ...
  TFile *outFile;
  if (noPtWeight) {
    outFile = new TFile(Form("totalHist_pp_%s_Zvtx%d_SF%d_etOpt%d_noPtWeight.root",szFinal.Data(),(int)useZvtxWeight,(int)useSF,etOpt),"RECREATE");
    cout << "totalHist_pp_"<<szFinal<<"_Zvtx"<<(int)useZvtxWeight<<"_SF"<<(int)useSF<<"_etOpt"<<etOpt<<"_noPtWeight.root has been created :) " <<endl;
  } else {
    outFile = new TFile(Form("totalHist_pp_%s_Zvtx%d_SF%d_etOpt%d.root",szFinal.Data(),(int)useZvtxWeight,(int)useSF,etOpt),"RECREATE");
    cout << "totalHist_pp_"<<szFinal<<"_Zvtx"<<(int)useZvtxWeight<<"_SF"<<(int)useSF<<"_etOpt"<<etOpt<<".root has been created :) " <<endl;
  }
