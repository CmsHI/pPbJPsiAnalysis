...
int draw_1D_eff(int MrapNpt=89, int isPA = 0, int accCutType =2, bool isPrompt = false, bool useZvtxWeight=false, bool useSF=false, char* szDir="dir_eff")
{
  ...
  char* szEffCut;
  if (accCutType==1) szEffCut="oldcut";
  else if (accCutType==2) szEffCut="newcut";
  else {cout << "select among accCutType = 1 or 2"<< endl; return 0; }	
	char* szPrompt;
  if (isPrompt) szPrompt = "PR";
  else szPrompt = "NP";	
	const char* szFinal = Form("%s_%s_%s_%s",szBinning,szPA,szPrompt,szEffCut);
	std::cout << "szFinal: " << szFinal << std::endl;

	TFile * f2D = new TFile(Form("EffAna_%s_Zvtx%d_SF%d.root",szFinal,(int)useZvtxWeight,(int)useSF));
	...
	c1->SaveAs(Form("%s/%s_Zvtx%d_SF%d_EffPt.pdf",szDir,szFinal,(int)useZvtxWeight,(int)useSF));
	
	//// save as a root file
	cout << "szFinal = " << szFinal << endl;
	TFile *fOut = new TFile(Form("%s/%s_Zvtx%d_SF%d_EffPt.root",szDir,szFinal,(int)useZvtxWeight,(int)useSF),"RECREATE");
	fOut->cd();
	for (Int_t iy = 0; iy < nRap; iy++) {
		h1D_EffPt[iy]->Write();
	}
