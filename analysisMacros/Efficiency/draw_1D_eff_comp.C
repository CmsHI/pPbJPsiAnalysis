...
void draw_1D_eff_comp(bool isPrompt = true, char* szDir = "dir_eff_comp_pr_np", bool isNoErr=false)
{
	gROOT->Macro("./Style.C");
	...
	f01 = new TFile(Form("./dir_eff/%s_EffPt.root",szName));
	f02 = new TFile(Form("./dir_eff/%s_EffPt.root",szName));
	...
	c1->SaveAs(Form("%s/%s_comp_eff.pdf",szDir,szSample));
	c2->SaveAs(Form("%s/%s_compRatio_eff.pdf",szDir,szSample));
