...
//TFile* fTnp_pp_new = new TFile("../Efficiency/tagAndProbe/output_pp_5eta_cutG_all_L2Single_v8.root","READ"); //20160803 (off8M, tagpt5, (L2Mu3 || L2Mu5 || L2Mu7_) : wrong )
TFile* fTnp_pp_new = new TFile("../Efficiency/tagAndProbe/output_pp_5eta_cutG_all_nominal_v10.root","READ"); //20160816 (off8M, tagpt5, (L2Mu3 ||L3Mu3) )
...
int rootAna_efficiency(int MrapNpt = 89, int isPA =0, int accCutType = 2, bool isPrompt = true, bool useZvtxWeight =false, bool useSF=true)
{
  using namespace std;
  int initev =0;
  //int nevt = -1; //all
  ...
  std::ifstream fctau(Form("../FittingResult/fitRes/summary_pA_%s_newcut_nominal/fit_ctauErrorRange",szBinning.Data()),std::ios::in);
  ...
  const int nrap = nRap;
  const int npt = nPt;
  string* yrange = new string[nrap]; //1st run string
...
//    cout << "iev = " << iev <<", buffer : " << myTree->GetEntry(iev) << endl;
    ...
    for (Int_t irqq=0; irqq<Reco_QQ_size; ++irqq) {
      ...
      int binmin = h2D_ctErrmin->FindBin(dimu_RECO->Rapidity(),dimu_RECO->Pt());
      float emin = h2D_ctErrmin->GetBinContent(binmin);
      int binmax = h2D_ctErrmin->FindBin(dimu_RECO->Rapidity(),dimu_RECO->Pt());
      float emax = h2D_ctErrmax->GetBinContent(binmax);
      #endif
