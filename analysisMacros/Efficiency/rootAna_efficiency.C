...
float getSF(float mupt1=0, float mueta1=0, float mupt2=0, float mueta2=0, int acctype=2); 

//read TNP plots for useSF
//// old TNP, isNewAccCut==0
//TFile* fTnpRate = new TFile("../Efficiency/tagAndProbe/tnpRate_nominal_fitRatio_5etaBin.root");
...
//int rootAna_efficiency_8rap9pt(char *strBinning = "8rap9pt", bool isPrompt = true, bool is1st = true, bool useCtErrCut =true, bool useSF=true, bool useZvtxWeight=true)
int rootAna_efficiency(int MrapNpt = 89, int isPA =0, int accCutType = 2, bool isPrompt = false, bool useZvtxWeight =false, bool useSF=false)
{
  using namespace std;
  int initev =0;
  //int initev =800000;
  ...
  if (MrapNpt==89)  {szBinning = "8rap9pt"; }
	...
	TFile* fZvtx;
	if (isPA==0 && accCutType==1) { fZvtx = new TFile(Form("../Efficiency/zVtx201603/zVtx_G1_%s_isPrompt%d_isNewAccCut0_isOfficial0.root",szPA,(int)isPrompt));
	} else if (isPA!=0 && accCutType==1) { fZvtx = new TFile(Form("../Efficiency/zVtx201603/zVtx_G1_%s_isPrompt%d_isNewAccCut0_isOfficial1.root",szPA, (int)isPrompt));
	} else if (isPA==0 && accCutType==2) { fZvtx = new TFile(Form("../Efficiency/zVtx201603/zVtx_G1_%s_isPrompt%d_isNewAccCut1_isOfficial0.root",szPA,(int)isPrompt));
	} else { fZvtx = new TFile(Form("../Efficiency/zVtx201603/zVtx_G1_%s_isPrompt%d_isNewAccCut1_isOfficial1.root",szPA, (int)isPrompt)); }
//	TFile* fZvtx = new TFile(Form("../Efficiency/zVtx201602/zVtx_G1_is1st1_isPrompt%d_isOldAccCut0_isOfficial1.root",(int)isPrompt));
	...
	if (isPA==0){
		...
		if (isPrompt) {
			f1 = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySample/tot_JPsiWithFSR_pa_1st_run_STARTHI53_V27-v1_noAccCut_sglTrig_genMatch-v2.root"); // official
			...
			f1 = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySample/tot_inclBtoJPsiMuMu_pa_1st_run_STARTHI53_V27-v1_noAccCut_sglTrig_genMatch-v2.root"); // official
		...
		if (isPrompt) {
			f1 = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySample/tot_JPsiWithFSR_pa_2nd_run_STARTHI53_V27-v1_noAccCut_sglTrig_genMatch-v1.root"); // official
			...
			f1 = new TFile("/home/songkyo/kyo/pPbDataSample/EfficiencySample/tot_inclBtoJPsiMuMu_pa_2nd_run_STARTHI53_V27-v1_noAccCut_sglTrig_genMatch-v1.root"); // official
	...
	char * dirName = Form("fitRes_%s",szBinning);
	std::ifstream fctau(Form("../Efficiency/%s/summary_%s/fit_ctauErrorRange",dirName,szPA),std::ios::in);
  ...
  ULong64_t           HLTriggers_PP; //jolla zzajeong.. : different type for pp and pA
  Int_t           HLTriggers_PA;
	Float_t					zVtx;
	Int_t           Reco_QQ_size;
	Int_t           Reco_QQ_sign[20];   //[Reco_QQ_size] : 0=+,- / 1=+/+, 2=-/-
	ULong64_t           Reco_QQ_trig_PP[20];   //[Reco_QQ_size] : different type for pp and pA
	Int_t           Reco_QQ_trig_PA[20];   //[Reco_QQ_size] 
...
//  myTree->SetBranchAddress("Reco_QQ_ctauErr", Reco_QQ_ctauErr, &b_Reco_QQ_ctauErr);
  ...
  myTree->SetBranchAddress("Gen_QQ_mumi_4mom", &Gen_QQ_mumi_4mom, &b_Gen_QQ_mumi_4mom);
	...
	h2D_Num_pt_y->Sumw2();
	h2D_Eff_pt_y->Sumw2();
	TH2D *h2D_Den_pt_y_fine = new TH2D("h2D_Den_pt_y_fine","",100,-2.5,2.5,100,0.,30.);
	TH2D *h2D_Num_pt_y_fine = new TH2D("h2D_Num_pt_y_fine","",100,-2.5,2.5,100,0.,30.);
	TH2D *h2D_Eff_pt_y_fine = new TH2D("h2D_Eff_pt_y_fine","",100,-2.5,2.5,100,0.,30.);
  ...
  TLorentzVector* dimu_GEN = new TLorentzVector; // GEN dimuon - NoCut
  TLorentzVector* jjjj_GEN = new TLorentzVector; // GEN Jpsi - NoCut
  TLorentzVector* jpsi_GEN = new TLorentzVector; //for GEN dimuon - Actual Denominator
  ...
  TLorentzVector* mumi_GEN = new TLorentzVector;
	...
	for(int iev=initev; iev<nevt; ++iev){
	  if(iev%100000==0) cout << ">>>>> EVENT " << iev << " / " << myTree->GetEntries() << " ("<<(int)(100.*iev/myTree->GetEntries()) << "%)" << endl;
		if (isPA==0 && isPrompt==true && (iev==882739 || iev ==884521 )) continue; //file corrupted?
		else if (isPA==0 && isPrompt==false && (iev == 226946 || iev==36800 || iev == 780741 || iev == 884521)) continue; //file corrupted?
	  //gDebug=2;
		myTree->GetEntry(iev);
//		cout << "iev = " << iev <<", buffer : " << myTree->GetEntry(iev) << endl;
	  ...
	  for (Int_t igqq=0; igqq<Gen_QQ_size; ++igqq) {
	    ...
	    *dimu_GEN = *mupl_GEN +  *mumi_GEN; // Gen dimuon pairs (for filling NoCut)
	    
	    if ( massCutWide(dimu_GEN->M())
			&& kineCut(mupl_GEN, accCutType)
			&& kineCut(mumi_GEN, accCutType)) {
				h2D_Den_pt_y->Fill(dimu_GEN->Rapidity(),dimu_GEN->Pt());
	    ...
	    //&& emin < Jpsi_Reco.theCtErr && Jpsi_Reco.theCtErr < emax
	   	...
	   	&& ( ( isPA==0 && (Reco_QQ_trig_PP[irqq]&1)==1 && (HLTriggers_PP&1)==1 ) 
			...
			&& kineCut(mumi_RECO, accCutType)) {
				//// TNP && vZtx weighting	
	  		tnpWeight = 1;
	    	if (useSF){ tnpWeight = getSF(mupl_RECO->Pt(),mupl_RECO->Eta(),mumi_RECO->Pt(),mumi_RECO->Eta(),accCutType);}
	...
	TFile *outFile = new TFile(Form("EffAna_%s_Zvtx%d_SF%d.root",szFinal, (int)useZvtxWeight,(int)useSF),"RECREATE");
	std::cout << "szFinal: " << szFinal << std::endl;
	outFile->cd();
	//h2D_ctErrmin->Write();
	//h2D_ctErrmax->Write();
...
float getSF(float mupt1, float mueta1, float mupt2, float mueta2, int acctype) { 
