...
*/	
  ...
  for (Int_t iy = 0; iy < nRap; iy++) {
	...
	c02->Modified();
	c02->Update();
	if (isMax) c02->SaveAs(Form("dir_fitSys/%s_fitSysHist_total_%s_etOpt%d_%s_upto%.0f_max.pdf",szPA.Data(),szBinning.Data(),etOpt,szPrompt.Data(),ymax));
	else c02->SaveAs(Form("dir_fitSys/%s_fitSysHist_total_%s_etOpt%d_%s_upto%.0f_rms.pdf",szPA.Data(),szBinning.Data(),etOpt,szPrompt.Data(),ymax));
