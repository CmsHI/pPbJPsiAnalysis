...
void draw_1D_fitSys_total_8rap9pt(int isPA=1, bool isPrompt=false, double ymax=100, bool isMax=true, int etOpt = 2)
{
	gROOT->Macro("../Style.C");
...
/*
  ...
  for (Int_t iy = 0; iy < nRap; iy++) {
	...
	c01->Modified();
	c01->Update();
	c01->SaveAs(Form("dir_fitSys/%s_fitSys_total_8rap9pt_etOpt%d_%s_upto%.0f.pdf",szPA.Data(),etOpt,szPrompt.Data(),ymax));
*/	
  //// hist
  ...
  for (Int_t iy = 0; iy < nRap; iy++) {
	...
	c02->Modified();
	c02->Update();
	if (isMax) c02->SaveAs(Form("dir_fitSys/%s_fitSysHist_total_8rap9pt_etOpt%d_%s_upto%.0f_max.pdf",szPA.Data(),etOpt,szPrompt.Data(),ymax));
	else c02->SaveAs(Form("dir_fitSys/%s_fitSysHist_total_8rap9pt_etOpt%d_%s_upto%.0f_rms.pdf",szPA.Data(),etOpt,szPrompt.Data(),ymax));
