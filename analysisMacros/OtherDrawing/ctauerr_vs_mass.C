...
void ctauerr_vs_mass(TString szPA = "pa", bool isSmall =false, double rapmin=-2.4, double rapmax=2.4, double ptmin=0.0, double ptmax=30.0)
{
  gROOT->Macro("../Style.C");
...
*/
 ...
 TCut accNewRecoPlus = "(TMath::Abs(Reco_QQ_mupl_4mom.Eta()) < 2.4 && ((TMath::Abs(Reco_QQ_mupl_4mom.Eta()) < 1.2 && Reco_QQ_mupl_4mom.Pt() >=3.3) || (1.2 <= TMath::Abs(Reco_QQ_mupl_4mom.Eta()) && TMath::Abs(Reco_QQ_mupl_4mom.Eta()) < 2.1 && Reco_QQ_mupl_4mom.Pt() >= 3.93-1.11*TMath::Abs(Reco_QQ_mupl_4mom.Eta())) || (2.1 <= TMath::Abs(Reco_QQ_mupl_4mom.Eta()) && Reco_QQ_mupl_4mom.Pt() >= 1.3)))";
  ...
  totalCut = trigCut && recoPairCut && massWide && ctauCut && rapRange && ptRange && accNewRecoPlus && accNewRecoMinus && muIDPlus && muIDMinus;
  ...
  TCanvas* c_tmp = new TCanvas("c_tmp","",300,300); c_tmp->SetGridx(); c_tmp->SetGridy();
	...
	TH2D* h2D_01 = new TH2D("h2D_01",";M_{#mu#mu} [GeV];J/#psi c#tau error [mm];",nbin,2.6,3.5,nbin,0,binmax);
	TH2D* h2D_02 = new TH2D("h2D_02",";M_{#mu#mu} [GeV];J/#psi c#tau error [mm];",nbin,2.6,3.5,nbin,0,binmax);
	TH2D* h2D_03 = new TH2D("h2D_03",";M_{#mu#mu} [GeV];J/#psi c#tau error [mm];",nbin,2.6,3.5,nbin,0,binmax);
	h2D_01->Sumw2();
	h2D_02->Sumw2();
	h2D_03->Sumw2();
	...
	c_tmp->cd();	
  if (isSmall) {
  	treeData->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_01",totalCut,"colz",nEntry);
  	//treePRMC->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_02",totalCut,"colz",nEntry);
  	//treeNPMC->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_03",totalCut,"colz",nEntry);
  }
  else {
  	treeData->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_01",totalCut,"colz");
  	//treePRMC->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_02",totalCut,"colz");
  	//treeNPMC->Draw("Reco_QQ_ctauErr:Reco_QQ_4mom.M()>>h2D_03",totalCut,"colz");
  }
/*	
	h2D_01->Scale(1,"width");	
	h2D_02->Scale(1,"width");	
	h2D_03->Scale(1,"width");	
	h2D_01->Scale(1./h2D_01->Integral());	
	h2D_02->Scale(1./h2D_02->Integral());	
	h2D_03->Scale(1./h2D_03->Integral());	
*/	
  SetHistStyle(h2D_01,1,0);
  SetHistStyle(h2D_02,4,0);
  SetHistStyle(h2D_03,3,0);
  h2D_02->SetFillColor(kGreen+3);
  h2D_02->SetFillStyle(3004);
  h2D_03->SetFillColor(kOrange+1);
  h2D_03->SetFillStyle(3005);
	
	TLegend *legUR = new TLegend(0.53, 0.80, 0.89, 0.95);
  SetLegendStyle(legUR);
	legUR->AddEntry(h2D_01,Form("%s Data",szPA.Data()),"pl");
	legUR->AddEntry(h2D_02,Form("%s MC prompt",szPA.Data()),"lf");
	legUR->AddEntry(h2D_03,Form("%s MC non-prompt",szPA.Data()),"lf");
  ...
  latex->SetTextSize(0.04);
	...
	h2D_01->Draw("colz");
//	h2D_02->Draw("hist");
//	h2D_03->Draw("hist same");
//	h2D_01->Draw("pe same");
// 	legUR->Draw();
//  latex->DrawLatex(0.55,0.75,Form("%s",rapArr.Data()));
//  latex->DrawLatex(0.55,0.70,Form("%s",ptArr.Data()));
	//c01->SaveAs(Form("dir_ctauerr/%s_ctauerr_rap%.2f-%.2f_pt%.2f-%.2f.pdf",szPA.Data(),rapmin,rapmax,ptmin,ptmax));
  ...
  latex->DrawLatex(0.55,0.70,Form("%s",ptArr.Data()));
 	legUR->Draw();
	c02->SaveAs(Form("dir_ctauerr/%s_ctauerr_log_rap%.2f-%.2f_pt%.2f-%.2f.pdf",szPA.Data(),rapmin,rapmax,ptmin,ptmax));
*/
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	c_ratio->cd();
	TH1D* hratio = (TH1D*)h2D_02->Clone("hratio"); 
	hratio->Divide(h2D_01);
