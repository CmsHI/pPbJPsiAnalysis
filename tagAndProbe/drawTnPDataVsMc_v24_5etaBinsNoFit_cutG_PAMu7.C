...
void drawTnPDataVsMc_v24_5etaBinsNoFit_cutG_PAMu7()
{
...
//Get graph{{{
	f1 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_data_allDirection_v24_nominalCond_5etaBins_cutG_all_PAMu7_v3.root");
	for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
		daTrkPt1[ieta] = (RooDataSet*)f1->Get(Form("tpTree/ptBin_eta%d/fit_eff",ieta));
	}
	f2 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_mc_allDirection_v24_nominalCond_5etaBins_cutG_all_PAMu7_v3.root");
  for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
...
//Define binning{{{
	...
	Double_t ptBinsArr5[10] = {1.3, 1.6, 2.2, 3.4, 4.8, 6, 9, 30};
...
//			if(ieta == 5 && ip == 3) yerr2 = yerr2/100;
/*
			if(ieta == 2 && ip == 1)
			{
				yerr1 = yerr1/2;
			...
			if(ieta == 2 && ip == 5) yerr1 = yerr1/2;
			if(ieta == 2 && ip == 2) yerr2 = yerr2/60;
			if(ieta == 4 && ip == 2) yerr2 = yerr2/100;
			if(ieta == 4 && ip == 3) yerr2 = yerr2/60;
			if(ieta == 4 && ip == 5) yerr2 = yerr2*3/1000;
			if(ieta == 4 && ip == 6) yerr2 = yerr2*3/100;
*/
...
//ratio{{{
		...
		if(ieta == 3)
		{
			...
			func[ieta]->SetParLimits(1, 3.0, 4.0);
			func[ieta]->SetParLimits(2, 0.0, 1.0);
			func[ieta]->SetParLimits(3, 3.0, 4.0);
			func[ieta]->SetParLimits(4, 1, 2);
		}
		if(ieta == 4)
		{
			func[ieta]->SetParLimits(0, -5, -4);
...
//save{{{
   c1->Update();
   c1->SaveAs("tnpSummaryPlot_nominpt_v24_5eta_cutG_all_PAMu7_v3_noErrorfix.pdf");
	TFile * output = new TFile("output_v24_5eta_cutG_all_PAMu7_v3_noErrorfix.root","recreate");
	for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
		eff_pt1[ieta]->Write();
		eff_pt2[ieta]->Write();
