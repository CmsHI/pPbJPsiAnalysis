...
void drawTnPDataVsMc_v27_5etaBinsNoFit_cutOLD_nominal_v6()
{
...
//Get graph{{{
	f1 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_data_allDirection_v27_nominalCond_5etaBins_cutOLD_all_nominal_v6.root");
	f2 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_mc_allDirection_v27_nominalCond_5etaBins_cutOLD_all_nominal_v6.root");
...
//efficiency{{{
	...
	drawText(  "     tag p_{T}>5.0",0.5,0.1);
	TLegend *leg = new TLegend(0.5562018,0.15,0.9975074,0.25,NULL,"brNDC");
...
//save{{{
   c1->Update();
   c1->SaveAs("tnpSummaryPlot_nominpt_v27_5eta_cutOLD_all_nominal_v6.pdf");
	TFile * output = new TFile("output_v27_5eta_cutOLD_all_nominal_v6.root","recreate");
	eff_eta1_1->Write();
	eff_eta2_1->Write();
	eff_rate->Write();
	//func->Write();
