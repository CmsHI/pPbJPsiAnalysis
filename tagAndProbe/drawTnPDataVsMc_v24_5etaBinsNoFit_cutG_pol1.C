...
void drawTnPDataVsMc_v24_5etaBinsNoFit_cutG_pol1()
{
...
//Define file and graph{{{
	...
	TGraphAsymmErrors *eff_pt1[20];//graph obtained from input
	TGraphAsymmErrors *eff_pt1_1[20];//refilled graph
	// mc
	TFile *f2; 
	RooDataSet *daTrkPt2[20];
	TGraphAsymmErrors *eff_pt2[20];//graph obtained from input
	TGraphAsymmErrors *eff_pt2_1[20];//refilled graph

	TGraphAsymmErrors *eff_rate[20];//efficiency ratio draw to canvas
	TF1* func[20];
//}}}
...
//Get graph{{{
	f1 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_data_allDirection_v24_nominalCond_5etaBins_cutG_all_pol1_v3.root");
	for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
		daTrkPt1[ieta] = (RooDataSet*)f1->Get(Form("tpTree/ptBin_eta%d/fit_eff",ieta));
	}
	f2 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_mc_allDirection_v24_nominalCond_5etaBins_cutG_all_pol1_v3.root");
  for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
...
//			if(ieta == 3 && ip == 0) yerr1 = yerr1/4;
//			if(ieta == 5 && ip == 1) yerr1 = yerr1/5;
...
//save{{{
   c1->Update();
   c1->SaveAs("tnpSummaryPlot_nominpt_v24_5eta_cutG_all_pol1_v3_noErrorfix.pdf");
	TFile * output = new TFile("output_v24_5eta_cutG_all_pol1_v3_noErrorfix.root","recreate");
	for ( int ieta = 1; ieta <= nEtaBin ; ieta++)
	{
		eff_pt1[ieta]->Write();
		eff_pt2[ieta]->Write();
