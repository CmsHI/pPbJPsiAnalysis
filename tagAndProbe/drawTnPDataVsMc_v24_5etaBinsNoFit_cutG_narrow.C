...
void drawTnPDataVsMc_v24_5etaBinsNoFit_cutG_narrow()
{
...
//Get graph{{{
	f1 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_data_allDirection_v24_nominalCond_5etaBins_cutG_all_narrow_v3.root");
	for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
		daTrkPt1[ieta] = (RooDataSet*)f1->Get(Form("tpTree/ptBin_eta%d/fit_eff",ieta));
	}
	f2 = new TFile("/home/kisoo/work/pPb_Jpsi/tagAndProbe/DataFile/outputEveryCut_mc_allDirection_v24_nominalCond_5etaBins_cutG_all_narrow_v3.root");
  for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
...
//save{{{
   c1->Update();
   c1->SaveAs("tnpSummaryPlot_nominpt_v24_5eta_cutG_all_narrow_v3_noErrorfix.pdf");
	TFile * output = new TFile("output_v24_5eta_cutG_all_narrow_v3_noErrorfix.root","recreate");
	for ( int ieta = 1; ieta<=nEtaBin ; ieta++)
	{
		eff_pt1[ieta]->Write();
		eff_pt2[ieta]->Write();
