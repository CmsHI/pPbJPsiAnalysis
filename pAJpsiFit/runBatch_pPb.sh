...
# Non-prompt MC
mc1=/afs/cern.ch/work/k/kyolee/private/pAJpsi_rooDataSet_20150910/outRoo_NPMC_pPb_mcTwoWay_cutG/outRoo_NPMC_pPb_mcTwoWay_cutG.root
# Prompt MC
mc2=/afs/cern.ch/work/k/kyolee/private/pAJpsi_rooDataSet_20150910/outRoo_PRMC_pPb_mcTwoWay_cutG/outRoo_PRMC_pPb_mcTwoWay_cutG.root
...
#      printf "cd -\n" >> $scripts/$work.csh
      ...
      if [ "$cent" == "0.0-100.0" ]; then
        if [ "$dphi" == "0.000-1.571" ]; then
          script="$executable -f $datasets $weight -m $mc1 $mc2 -v $mSigF $mBkgF -d $prefix -r $eventplane $usedPhi -u $resOpt -a $anaBct $ctauBkg -b $ispbpb $isPEE $is2Widths -p $pt -y $rap -t $cent -s $dphi -l $ctaurange -x $runOpt $readct $cterrfile -z $fracfree -j $isMultiplicity -h $ethf -n $ntrk >& $work.log;"
          echo $script >> $scripts/$work.csh
        elif [ "$dphi" != "0.000-1.571" ]; then
          script="$executable -f $datasets $weight -m $mc1 $mc2 -v $mSigF $mBkgF -d $prefix -r $eventplane $usedPhi -u $resOpt -a 2 $ctauBkg -b $ispbpb $isPEE $is2Widths -p $pt -y $rap -t $cent -s 0.000-1.571 -l $ctaurange -x $runOpt $readct $cterrfile -z $fracfree -j $isMultiplicity -h $ethf -n $ntrk >& $workPHI.log;"
          echo $script >> $scripts/$work.csh
          script="$executable -f $datasets $weight -m $mc1 $mc2 -v $mSigF $mBkgF -d $prefix -r $eventplane $usedPhi -u $resOpt -a $anaBct $ctauBkg -b $ispbpb $isPEE $is2Widths -p $pt -y $rap -t $cent -s $dphi -l $ctaurange -x $runOpt $readct $cterrfile -z $fracfree -j $isMultiplicity -h $ethf -n $ntrk >& $work.log;"
          echo $script >> $scripts/$work.csh
        fi
        ...
        if [ "$dphi" == "0.000-1.571" ]; then
          script="$executable -f $datasets $weight -m $mc1 $mc2 -v $mSigF $mBkgF -d $prefix -r $eventplane $usedPhi -u $resOpt -a 2 $ctauBkg -b $ispbpb $isPEE $is2Widths -p $pt -y $rap -t 0.0-100.0 -s 0.000-1.571 -l $ctaurange -x $runOpt $readct $cterrfile -z $fracfree -j $isMultiplicity -h $ethf -n $ntrk >& $workMB.log;"
          echo $script >> $scripts/$work.csh
          script="$executable -f $datasets $weight -m $mc1 $mc2 -v $mSigF $mBkgF -d $prefix -r $eventplane $usedPhi -u $resOpt -a $anaBct $ctauBkg -b $ispbpb $isPEE $is2Widths -p $pt -y $rap -t $cent -s $dphi -l $ctaurange -x $runOpt $readct $cterrfile -z $fracfree -j $isMultiplicity -h $ethf -n $ntrk >& $work.log;"
          echo $script >> $scripts/$work.csh
        elif [ "$dphi" != "0.000-1.571" ]; then
