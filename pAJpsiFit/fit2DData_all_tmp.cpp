...
int main (int argc, char* argv[]) {
  ...
  if (!inOpt.prange.compare("0.0-3.0") || !inOpt.prange.compare("0.0-1.5") || !inOpt.prange.compare("1.5-3.0")){ lmin = 3.0; lmax = 5.0; } // KYO
  ...
  if (fInMC2.IsZombie()) { cout << "CANNOT open MC2(PR) root file\n"; return 1; }
  fInMC2.cd();
  //RooDataSet *dataMC2 = (RooDataSet*)fInMC2.Get("dataJpsi");
  RooDataSet *dataMC2;
  if (inOpt.doWeight == 1) {
    dataMC2 = (RooDataSet*)fInMC2.Get("dataJpsiWeight");  //Weighted
    cout << "## WEIGHTED MC2(PR) is used\n";
  } else {
  ...
  if (!inOpt.rpmethod.compare("etHFm")) { // use this option - KYO
    sprintf(reduceDS2,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && Jpsi_Y>=%.2f && Jpsi_Y<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f && Ntracks >=%.1f && Ntracks < %.1f && SumET_HFEta4 >=%.2f && SumET_HFEta4 < %.2f",
        pmin,pmax,ymin,ymax,-lmin,lmax,psmin,psmax,ntrmin,ntrmax,etmin,etmax);
    sprintf(reduceDS2MC,
    ...
    sprintf(reduceDS2,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && Jpsi_Y>=%.2f && Jpsi_Y<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f && Ntracks >=%.1f && Ntracks < %.1f && SumET_HFEta4 >=%.2f && SumET_HFEta4 < %.2f",
        pmin,pmax,-ymax,-ymin,-lmin,lmax,psmin,psmax,ntrmin,ntrmax,etmin,etmax);
    sprintf(reduceDS2MC,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && Jpsi_Y>=%.2f && Jpsi_Y<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f",
    ...
    sprintf(reduceDS,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && Jpsi_Y>=%.2f && Jpsi_Y<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_CtErr>=%.3f && Jpsi_CtErr<%.3f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f && Ntracks >=%.1f && Ntracks < %.1f && SumET_HFEta4 >=%.2f && SumET_HFEta4 < %.2f",
        pmin,pmax,-ymax,-ymin,-lmin,lmax,errmin,errmax,psmin,psmax,ntrmin,ntrmax,etmin,etmax);
    sprintf(reduceDSMC,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && Jpsi_Y>=%.2f && Jpsi_Y<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_CtErr>=%.3f && Jpsi_CtErr<%.3f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f",
        pmin,pmax,-ymax,-ymin,-lmin,lmax,errmin,errmax,psmin,psmax);
  } else { // absolute rap
    sprintf(reduceDS2,
        "Jpsi_Pt>=%.2f && Jpsi_Pt<%.2f && TMath::Abs(Jpsi_Y)>=%.2f && TMath::Abs(Jpsi_Y)<%.2f && Jpsi_Ct>=%.2f && Jpsi_Ct<%.2f && Jpsi_dPhi>=%.3f && Jpsi_dPhi<%.3f && Ntracks >=%.1f && Ntracks < %.1f && SumET_HFEta4 >=%.2f && SumET_HFEta4 < %.2f",
        pmin,pmax,ymin,ymax,-lmin,lmax,psmin,psmax,ntrmin,ntrmax,etmin,etmax);
    sprintf(reduceDS2MC,
  ...
  if (!inOpt.prange.compare("0.0-3.0") || !inOpt.prange.compare("0.0-1.5") || !inOpt.prange.compare("1.5-3.0")){ ws->var("Jpsi_Mass")->setBins(100);} // KYO
...
//  ws->var("Jpsi_Ct")->setBinning(rbCorser);  //corser
	if (!inOpt.prange.compare("0.0-3.0") || !inOpt.prange.compare("0.0-1.5") || !inOpt.prange.compare("1.5-3.0")){ws->var("Jpsi_Ct")->setBins(100);}  // KYO
...
//							ws->var("coeffGaus")->setRange(0.22,0.78);
...
//							ws->var("coeffGaus")->setVal(0.4);
						...
						if (!inOpt.prange.compare("0.0-3.0")){
//							ws->var("coeffGaus")->setRange(0.68,0.89);
...
//							ws->var("coeffGaus")->setRange(0.28,0.78);
//							ws->var("coeffGaus")->setVal(0.5);
					...
					////// not used
						...
						if (!inOpt.prange.compare("10.0-30.0")){
							//ws->var("sigmaSig1")->setRange(0.01,0.04);
							//ws->var("sigmaSig1")->setVal(0.02);
							//ws->var("sigmaSig2")->setRange(0.025,0.05);
							//ws->var("sigmaSig2")->setVal(0.04);
					...
					if (!inOpt.yrange.compare("1.97-2.4")) {
						if (!inOpt.prange.compare("0.0-3.0")) {
							//ws->var("sigmaSig1")->setRange(0.030,0.060);
							//ws->var("sigmaSig1")->setVal(0.05);
							//ws->var("sigmaSig2")->setRange(0.01,0.045);
						//	ws->var("sigmaSig2")->setRange(0.01,0.060);
					//		ws->var("sigmaSig2")->setVal(0.04);
							ws->var("lambdap")->setRange(0.05,1.5);
							ws->var("lambdam")->setRange(0.02,1.5);
							ws->var("lambdam")->setVal(0.79);
...
//							ws->var("coeffGaus")->setRange(0.52,0.85);
...
//							ws->var("coeffGaus")->setVal(0.5);	
		...
		//					ws->var("sigmaSig2")->setVal(0.02);
        			...
        			ws->var("sigmaResSigW")->setRange(0.0,2.9);
...
//							ws->var("coeffGaus")->setVal(0.8);
		...
		//					ws->var("sigmaSig2")->setVal(0.02);
    	 	     	sprintf(funct,"SUM::sigMassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
						...
						else if (!inOpt.prange.compare("4.0-5.0")) {
		//					ws->var("sigmaSig1")->setRange(0.03,0.055);
		//					ws->var("sigmaSig1")->setVal(0.04);
		//					ws->var("sigmaSig2")->setRange(0.015,0.045);
		//					ws->var("sigmaSig2")->setVal(0.02);
...
//							ws->var("coeffGaus")->setRange(0.48,0.71);
//							ws->var("coeffGaus")->setVal(0.65);	
		...
		//					ws->var("sigmaSig2")->setVal(0.02);
					...
					//not used 
					else if (!inOpt.yrange.compare("-2.4--1.46")) {
...
//          ws->factory("Gaussian::tempresGN(CtWeighted,meanResSigW,sigmaResSigN)");
  ...
  //      ws->var("sigmaResSigM")->setConstant(kTRUE);
      ...
      drawCtauSBPlots(ws, redDataSB, redDataSBL, redDataSBR, binDataCtErrSB, fitSB, fitSBL, fitSBR, inOpt, &UnNormChi2_side, &nFitParam_side, &nFullBinsPull_side, &Dof_side, &Chi2_side); //  KYO - 0316
    }
...
/*		if (inOpt.isMultiplicity == 0) { // no multiplicity
		...
		// Special bin setting 20150213 - KYO 
		if (inOpt.isMultiplicity == 0) { // no multiplicity
			if (inOpt.isPbPb == 2) {
				if (!inOpt.yrange.compare("-2.4--1.97")) {
					if (!inOpt.prange.compare("0.0-3.0") || !inOpt.prange.compare("7.5-8.5") || !inOpt.prange.compare("8.5-10.0") ) {
						ws->var("sigmaResSigW")	->setConstant(kTRUE);
						ws->var("meanResSigW")	->setConstant(kFALSE);
					}
				}
				else if (!inOpt.yrange.compare("-1.97--1.37")) {
					if (!inOpt.prange.compare("4.0-5.0") || !inOpt.prange.compare("5.0-6.5") ) {
						ws->var("sigmaResSigW")	->setConstant(kTRUE);
						ws->var("meanResSigW")	->setConstant(kFALSE);
					}
				}
					...
					if (!inOpt.prange.compare("4.0-5.0") || !inOpt.prange.compare("6.5-7.5") ) {
						ws->var("sigmaResSigW")	->setConstant(kTRUE);
						ws->var("meanResSigW")	->setConstant(kFALSE);
					}
				}
			}
			else if (inOpt.isPbPb == 3) { // no multiplicity
				if (!inOpt.yrange.compare("1.97-2.4")) {
					if (!inOpt.prange.compare("0.0-3.0") || !inOpt.prange.compare("7.5-8.5") || !inOpt.prange.compare("8.5-10.0") ) {
						ws->var("sigmaResSigW")	->setConstant(kTRUE);
						ws->var("meanResSigW")	->setConstant(kFALSE);
...
//    drawCtauFitPlotsSignals(ws, redDataCut, redDataSB, redMCCutNP, NSigPR_fin, NSigNP_fin, inOpt);
    ...
    drawCtauFitPlots(ws, redDataCut, binDataCtErr, NSigNP_fin, NBkg_fin, fit2D, inOpt, &UnNormChi2_time, &nFitParam_time, &nFullBinsPull_time, &Dof_time, &Chi2_time); // KYO 0316 -time
      ...
      case '-':{
        switch (tmpargv[1]) {
          ...
          case 'm':
            ...
            cout << "MC data file 1 (NP) : " << opt.FileNameMC1 << endl;
            cout << "MC data file 2 (PR) : " << opt.FileNameMC2 << endl;
            break;
          case 'v':
...
// KYO -0316
void drawCtauSBPlots(RooWorkspace *ws, RooDataSet *redDataSB, RooDataSet *redDataSBL, RooDataSet *redDataSBR, RooDataHist *binDataCtErrSB, RooFitResult *fitSB, RooFitResult *fitSBL, RooFitResult *fitSBR, InputOpt &opt, double* UnNormChi2_side_t, int* nFitParam_side_t, int* nFullBinsPull_side_t, int* Dof_side_t,double* Chi2_side_t) {
  TLatex *t = new TLatex();  t->SetNDC();  t->SetTextAlign(12);
  char reduceDS[512];
  string titlestr;
  double unNormChi2;
...
/// KYO 20150316
void drawMassPlotsWithB(RooWorkspace *ws, RooDataSet* redDataCut, float NSigNP_fin, float NBkg_fin, RooFitResult *fitM, InputOpt &opt, double* UnNormChi2_mass_t, int* nFitParam_mass_t, int* nFullBinsPull_mass_t, int* Dof_mass_t,double* Chi2_mass_t) {
...
// KYO 0316
void drawCtauFitPlots(RooWorkspace *ws, RooDataSet *redDataCut, RooDataHist* binDataCtErr, float NSigNP_fin, float NBkg_fin, RooFitResult *fit2D, InputOpt &opt, double* UnNormChi2_time_t, int* nFitParam_time_t, int* nFullBinsPull_time_t, int* Dof_time_t,double* Chi2_time_t) {
  char reduceDS[512];
  string titlestr;
