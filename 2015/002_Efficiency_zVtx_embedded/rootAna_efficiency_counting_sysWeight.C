...
int rootAna_efficiency_counting_sysWeight(char *strBinning = "8rap9pt", bool isPrompt = false, bool is1st = true, bool isEmbedded = false, bool useCtErrRangeEff =true, bool useDataDrivenEff=true, bool useZvtxWeightStep1 = false, bool useZvtxWeightStep2=false){
...
//yeonju
    ...
    if(is1st){
        ...
        h2D_Eff_weight_pt_y = new TH2D("h2D_Eff_weight_pt_y","",nYBins,yBinsArr_pPb,nPtBins,ptBinsArr);
        //h2D_Eff_diff_pt_y = new TH2D("h2D_Eff_diff_pt_y","",nYBins,yBinsArr_pPb,nPtBins,ptBinsArr);
        h2D_Eff_err_pt_y = new TH2D("h2D_Eff_err_pt_y","",nYBins,yBinsArr_pPb,nPtBins,ptBinsArr);
    }
    ...
    TFile* toyFile = new TFile(Form("ToyGaussian_isPrompt%d_%s.root",(int)isPrompt,run_st.Data()));
    TH1D* hWeight[nYBins];
    TF1* Func[nYBins];
...
//weighting factor for Eff. Sys. by Yeonju
        ...
        w_toy = Func[yflag]->Eval(Jpsi_Reco.thePt);
        if(i<100) cout << " pt : " << Jpsi_Reco.thePt << ", y : " << Jpsi_Reco.theRapidity << ", weigt : " << w_toy << ", " << st_EffRange <<  endl;
	    ...
	    && minylab<=Jpsi_Reco.theRapidity && Jpsi_Reco.theRapidity < maxylab) {
	  	...
	  	if (yn_reco == true){ 
	    	if (useDataDrivenEff){
				...
				h2D_Num_weight_pt_y->Fill(Jpsi_Reco.theRapidity,Jpsi_Reco.thePt, effWeight*zWeight01*zWeight02*w_toy);
			}
	  ...
	  for (Int_t igqq=0; igqq<Gen_QQ_size; ++igqq) {
	    ...
	    *JP_Gen_tmp = *m1P_Gen +  *m2P_Gen; // Gen dimuon pairs (for filling NoCut)
	    ...
	    if (yn_gen == true){
				...
				h2D_Den_weight_pt_y->Fill(Jpsi_Gen.theRapidity,Jpsi_Gen.thePt,zWeight01*zWeight02*w_toy);
	    } // end of yn_gen
	...
	// --- save as a root file
	TFile *outFile = new TFile(Form("EffSys_EffCounting_%s_useCtErr_%d_useDataDriven_%d_useZvtxStep1_%d_Step2_%d.root",strName, (int)useCtErrRangeEff ,(int)useDataDrivenEff, (int)useZvtxWeightStep1, (int)useZvtxWeightStep2),"RECREATE");
	std::cout << "strName: " << strName << std::endl;
	outFile->cd();
