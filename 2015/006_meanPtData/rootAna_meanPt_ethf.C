...
int rootAna_meanPt_ethf(char *strBin = "6rap3pt"){
	...
	Double_t etArr[] = {0.0, 20., 30., 120.0}; // 6rap3pt
	...
	/////////////////////////////////////////////////////////////	
	////////////// cut definitions
	TCut trigCut = "( (Reco_QQ_trig&1)==1 && (HLTriggers&1)==1 )"; // double mu open
...
//	TCut massRange = "Reco_QQ_4mom.M() >2.6 && Reco_QQ_4mom.M() < 3.5";
	...
	for (int in=0; in<nEt; in++){
		for (int ir=0; ir<nRap; ir++ ) {
			for (int ipt=0; ipt<nPt; ipt++) {
				etRange[in][ir][ipt] = Form("SumET_HFplusEta4+SumET_HFminusEta4>%.1f && SumET_HFplusEta4+SumET_HFminusEta4<%.1f",etArr[in] , etArr[in+1]);
				...
				cout << "etRange = " <<etRange[in][ir][ipt] << endl;
				...
				totalCutPbp[in][ir][ipt] = trigCut && recoCut && massRange && ctauRange && accRecoPlus && accRecoMinus && rapRangePbp[in][ir][ipt] && ptRange[in][ir][ipt] && etRange[in][ir][ipt];
				totalCutpPb[in][ir][ipt] = trigCut && recoCut && massRange && ctauRange && accRecoPlus && accRecoMinus && rapRangepPb[in][ir][ipt] && ptRange[in][ir][ipt] && etRange[in][ir][ipt];
			}
		}
	}
	...
	for (int in=0; in<nEt; in++){
		for (int ir=0; ir<nRap; ir++ ) {
	 		cmulti[in][ir]= new TCanvas(Form("cmulti_%d_%d",in,ir),Form("%s",rapstrArr[ir].c_str()),900,300);
			cmulti[in][ir]->Divide(3,1);
			for (int ipt=0; ipt<nPt; ipt++) {
...
//				treeDatapPb->Draw(Form("Reco_QQ_4mom.Pt()>>+%s",hMeanPt[in][ir][ipt]->GetName()),totalCutpPb[in][ir][ipt],"",nEntry);
				...
				latex->DrawLatex(0.54, 0.80, etstrArr[in].c_str());
				latex->DrawLatex(0.54, 0.73, Form("<p_{T}> = %.2f (GeV/c)",meanVal[in][ir][ipt]));
				//c1->Update();
