...
void draw_1D_crossSection_pt_kyo01(char* dirName = "8rap9pt", int runCode=0, bool isScale = false,bool isLog = false)
{
...
//	TLegend *legUR = new TLegend(0.52, 0.55, 0.86, 0.92); //upper left
	...
	c_pr_fw->SaveAs(Form("cross_%s_kyo01/crossSection_pt_pr_fw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
	...
	c_pr_bw->SaveAs(Form("cross_%s_kyo01/crossSection_pt_pr_bw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
	...
	c_np_fw->SaveAs(Form("cross_%s_kyo01/crossSection_pt_np_fw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
	...
	c_np_bw->SaveAs(Form("cross_%s_kyo01/crossSection_pt_np_bw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
	...
	///////////////////////////////////////////////////////////////////
	// save as a root file
	TFile *outFile = new TFile(Form("cross_%s_kyo01/crossSection_pt_isLog%d_isScale%d.root",dirName,(int)isLog,(int)isScale),"RECREATE");
	outFile->cd();
	for (Int_t iy = 0; iy < nRap; iy++) {
