...
void draw_AccDist_test(bool isPrompt=false, bool isPbp=false){
	using namespace std;
...
//        formPtArr(ptArrNum[ipt], ptArrNum[ipt+1], &ptArr[ipt]);
    ...
    for(int iy=0;iy<nYBins;iy++){
        for(int ipt=0;ipt<nPtBins;ipt++){
            ...
            hAccCompBin_tot[iy][ipt]->SetLabelSize(0.05,"X");
            hAccCompBin_tot[iy][ipt]->SetLabelOffset(0.05,"X");
    ...
    for(int iy=0;iy<nYBins;iy++){
        ...
        for(int ipt=0;ipt<nPtBins;ipt++){
            ...
            hAccCompBin_tot[iy][ipt]->Draw("hist");
            jumSun(AccCent[iy][ipt],0,AccCent[iy][ipt],8000,4);
            latex->DrawLatex(0.53,0.25,sampleName);
            latex->DrawLatex(0.56,0.68,Form("%s",rapArr[iy].c_str()));
            latex->DrawLatex(0.56,0.88,Form("%s",ptArr[iy].c_str()));
    ...
    for(int ifile=0;ifile<nFile;ifile++){
        can[ifile]->SaveAs(Form("testpdf/AccDist_isPrompt%d_isPbp%d_ifile%d.pdf",(int)isPrompt,(int)isPbp,ifile));
    }
    can_tot->SaveAs(Form("testpdf/AccDist_isPrompt%d_isPbp%d_tot.pdf",(int)isPrompt,(int)isPbp));
    for(int iy=0;iy<nYBins;iy++){
        can_tot_iy[iy]->SaveAs(Form("testpdf/AccDist_isPrompt%d_isPbp%d_tot_rapidity%d.pdf",(int)isPrompt,(int)isPbp,iy));
    } 

    TFile* outf = new TFile(Form("testdraw_AccDist_isPrompt%d_isPbp%d.root",(int)isPrompt,(int)isPbp),"recreate");
    outf->cd();
    for(int iy=0;iy<nYBins;iy++){
        can_tot_iy[iy]->Write();
        for(int ipt=0;ipt<nPtBins;ipt++){
