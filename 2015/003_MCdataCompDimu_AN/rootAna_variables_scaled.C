...
int rootAna_variables_scaled(char *stringA = "20140301"){
...
//		if (i!=1) continue;
		...
		else if (i==3) hDataPbp[i]->SetAxisRange(0.0,0.045,"Y");
		hDataPbp[i]->Draw("pe");
		...
		c1[i]->SaveAs(Form("Variables/variables_%s_%s_scaled.pdf",histName[i].Data(),stringA));
		c1[i]->SaveAs(Form("Variables/variables_%s_%s_scaled.png",histName[i].Data(),stringA));
		//c1[i]->Clear();
		legUR->Clear();
		legUM->Clear();
		legUL->Clear();
	...
	//Save as a root file
	TFile* outFile = new TFile("Variables/variables_scaled.root","RECREATE");
	outFile->cd();
	for (int i=0; i<nHist; i++) {
		hDataPbp[i]->Write();
		hSigPbp[i]->Write();
