...
//		gCross_pr_sys[fwrap_init]->SetMaximum(350.);
	...
	c_pr_fw->SaveAs(Form("cross_%s/crossSection_pt_integ_pr_fw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
...
//		latex->DrawLatex(0.59, 0.50, "Global uncertainty : 3.6 \%");
	...
	c_pr_bw->SaveAs(Form("cross_%s/crossSection_pt_integ_pr_bw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
...
//		latex->DrawLatex(0.59, 0.50, "Global uncertainty : 3.6 \%");
	...
	c_np_fw->SaveAs(Form("cross_%s/crossSection_pt_integ_np_fw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
...
//		latex->DrawLatex(0.59, 0.50, "Global uncertainty : 3.6 \%");
	...
	c_np_bw->SaveAs(Form("cross_%s/crossSection_pt_integ_np_bw_isLog%d_isScale%d.pdf",dirName,(int)isLog,(int)isScale));
	legUR->Clear();
	...
	///////////////////////////////////////////////////////////////////
	// save as a root file
	TFile *outFile = new TFile(Form("cross_%s/crossSection_pt_integ_isLog%d_isScale%d.root",dirName,(int)isLog,(int)isScale),"RECREATE");
	outFile->cd();
